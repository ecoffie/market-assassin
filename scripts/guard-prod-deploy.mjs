...
/**
 * Git env vars that REDIRECT git at a different repository.
 *
 * git exports these into hook processes — a `pre-push` hook runs with `GIT_DIR` set.
 * Inheriting them makes every `git` call below read THAT repo instead of the directory
 * the guard was pointed at, which is precisely the "you inspected the wrong tree" error
 * this guard exists to prevent. Measured: the real-checkout tests passed on a manual
 * hook run and failed under an actual `git push` for exactly this reason.
 */
