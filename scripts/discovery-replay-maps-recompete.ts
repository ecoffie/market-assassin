...
export const MAPS_RECOMPETE_CLASSES: Record<string, { cls: 'canonical_correction' | 'expected_surface_policy' | 'unexpected_regression'; why: string }> = {
  ...
  'pam': { cls: 'canonical_correction', why: 'Old %pam% substring (PAMELA, PAMUNKEY) — 5 dropped, none mean PAM. Canonical word-bounded → 0 ≡ MCP.' },
  'market research': { cls: 'canonical_correction', why: 'Old whole-phrase ILIKE found 0; canonical both-qualifier match → 4 ("Marketing Research") ≡ MCP.' },
  'management': { cls: 'canonical_correction', why: '≡ MCP (4,493). Old ILIKE on incumbent/NAICS-desc/agency = 1,925; +2,569 matched ONLY via awarding_sub_agency names (Bureau of Land Management, DCMA) — canonical RECOMPETE_TEXT_COLS include buyer names. Recorded as a shared-layer limitation, not changed here.' },
  '541512 -computers': { cls: 'canonical_correction', why: 'Old keyword-ILIKE on the literal text → 0. Now NAICS 541512 minus computers → 4,246 ≡ MCP.' },
  'USDA -computers': { cls: 'canonical_correction', why: 'Old literal text → 0. Now USDA buyers minus computers → 4,581 ≡ MCP.' },
