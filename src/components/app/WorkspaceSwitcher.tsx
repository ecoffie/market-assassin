...
export default function WorkspaceSwitcher({
    ...
    return (
      <div className="px-2 py-2">
        <button
          ...
          className="w-full flex items-center justify-center p-2 rounded-lg bg-surface/50 hover:bg-surface transition-colors"
  ...
  return (
    <div ref={dropdownRef} className="px-4 py-2 relative">
      ...
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg bg-surface/50 hover:bg-surface transition-colors text-left"
      >
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute left-4 right-4 top-full mt-1 py-1 bg-ground border border-hairline rounded-lg shadow-xl z-50">
          <div className="px-3 py-1.5 text-[10px] font-medium text-faint uppercase tracking-wider">
            Workspaces
          </div>
          {workspaces.map(workspace => (
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-surface transition-colors ${
                workspace.id === currentWorkspaceId ? 'bg-surface/50' : ''
              }`}
            >
              <span className="text-sm">
              ...
              {workspaces
                ...
                .map(workspace => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm text-muted truncate">{workspace.name}</div>
