...
export default function ProposalChat({
  ...
  return (
    <section className="bg-ground border border-purple-500/30 rounded-xl overflow-hidden grid grid-cols-1 lg:grid-cols-[1fr_300px]">
      {/* CENTER — chat */}
      <div className="flex flex-col p-5" style={{ minHeight: 480 }}>
        <div className="flex items-center justify-between mb-2">
          ...
          {sources.length > 0 && <span className="text-[11px] text-faint">drafting from {sources.length} source{sources.length === 1 ? '' : 's'}</span>}
        </div>

        <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-3 pr-1" style={{ maxHeight: 380 }}>
          {messages.length === 0 ? (
            <div className="text-sm text-muted">
              <p className="mb-1 text-ink-soft font-medium">Mindy already knows this bid.</p>
              <p className="mb-3">Unlike a blank-slate chatbot, she reads <span className="text-ink-soft">this solicitation</span>, its extracted requirements, your Vault, and winning-proposal examples. Best for questions about <span className="text-ink-soft">this opportunity</span> and drafting grounded in your real info.</p>
              <p className="text-[11px] uppercase tracking-wider text-faint mb-1.5">Try asking</p>
              <div className="flex flex-wrap gap-2">
                {STARTERS.map(s => (
                  <button key={s} onClick={() => send(s)} className="rounded-lg border border-hairline bg-surface/60 px-3 py-1.5 text-xs text-ink-soft hover:border-purple-500/50 hover:text-white">
                    {s}
                  </button>
                ))}
              </div>
            ...
            <div key={i} className={m.role === 'user' ? 'text-right' : ''}>
              <div className={`inline-block max-w-[92%] rounded-lg px-3 py-2 text-sm whitespace-pre-wrap text-left ${m.role === 'user' ? 'bg-purple-600 text-white' : 'bg-surface text-slate-200'}`}>
                {m.content || (streaming && i === messages.length - 1 ? '…' : '')}
              </div>
              {m.role === 'assistant' && m.content && !streaming && (
                <button onClick={() => copy(m.content)} className="mt-1 block text-[11px] text-faint hover:text-ink-soft">Copy</button>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="mt-3 flex gap-2">
          <input
            ...
            disabled={streaming}
            className="flex-1 h-10 px-3 bg-surface border border-hairline rounded-lg text-white text-sm placeholder-faint focus:border-purple-500 focus:outline-none disabled:opacity-50"
          />
          <button onClick={() => send(input)} disabled={streaming || !input.trim()} className="h-10 px-5 bg-purple-600 hover:bg-purple-500 disabled:bg-input text-white text-sm font-medium rounded-lg">
            {streaming ? '…' : 'Send'}
          </button>
        </div>
      </div>
      ...
      <aside className="border-t lg:border-t-0 lg:border-l border-surface bg-ground-deep/40 p-4 flex flex-col gap-5">
        {/* Files */}
        <div>
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-xs font-semibold uppercase tracking-wider text-muted">Files in context</h3>
          ...
          <p className="text-[11px] text-faint mb-2">Mindy reads everything here. Add or remove to control what she sees.</p>
          <div className="space-y-1.5">
            ...
            {files.map(f => (
              <div key={f.fileName} className="group flex items-center justify-between gap-2 rounded bg-surface/60 px-2 py-1.5 text-xs">
                <span className="inline-flex min-w-0 items-center gap-1 truncate text-ink-soft" title={f.fileName}><FileText className="h-3 w-3 shrink-0" strokeWidth={2} /> {f.fileName}</span>
            ...
            {hasVault && (
              <div className="flex items-center gap-2 rounded bg-surface/40 px-2 py-1.5 text-xs text-muted">
        ...
        {/* Instructions */}
        <div>
          <h3 className="text-xs font-semibold uppercase tracking-wider text-muted mb-1">Instructions</h3>
          <p className="text-[11px] text-faint mb-2">Standing guidance applied to every answer.</p>
          <textarea
            value={instructions}
            onChange={e => setInstructions(e.target.value)}
            placeholder="e.g. Write in plain language. Always cite the RFP section. Keep sections under 2 pages."
            rows={4}
            className="w-full bg-surface border border-hairline rounded-lg px-2 py-2 text-xs text-white placeholder-faint focus:border-purple-500 focus:outline-none resize-none"
          />
        </div>
      </aside>
    </section>
