...
export default function OsbpSmbResearchPanel({ email }: Props) {
  ...
  const badge = (r: Row) => r.psc_exact ? 'bg-emerald-500/20 text-emerald-300'
    : r.psc_family ? 'bg-blue-500/20 text-blue-300' : 'bg-input text-ink-soft';
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-ground border border-surface rounded-xl p-5 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label className="block text-xs text-faint mb-1">PSC Code <span className="text-emerald-400/70">(what you\'re buying — best match)</span></label>
          ...
          <div>
            <label className="block text-xs text-faint mb-1">NAICS Code <span className="text-slate-600">(widens the net)</span></label>
          ...
          <div>
            <label className="block text-xs text-faint mb-1">Max federal $ <span className="text-slate-600">(smaller = smaller firms)</span></label>
            <select value={maxM} onChange={e => setMaxM(e.target.value)}
              className="w-full px-3 py-2 bg-surface border border-hairline rounded-lg text-white text-sm focus:border-blue-500 focus:outline-none">
          ...
          <div className="flex items-end">
            <button onClick={() => run(0, false)} disabled={loading}
              className="w-full px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-input text-white font-medium rounded-lg transition-colors">
      ...
      {rows && (
        <div className="bg-ground border border-surface rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-surface flex items-center justify-between gap-3 flex-wrap">
          ...
          {rows.length === 0 && (
            <div className="p-8 text-center text-faint text-sm">No matches. Try a broader PSC family or NAICS prefix.</div>
          )}
          <div className="divide-y divide-slate-800">
            {rows.map(r => (
              <div key={r.recipient_uei} className="p-4 flex items-start justify-between gap-4 hover:bg-surface/40">
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className="text-xs text-faint">{r.award_count.toLocaleString()} contracts · {r.agency_count} agencies</div>
          ...
          {rows.length < total && (
            <div className="p-4 border-t border-surface text-center">
              <button onClick={() => run(offset + PAGE, true)} disabled={loading}
                className="px-4 py-2 bg-surface hover:bg-input text-slate-200 text-sm rounded-lg transition-colors">
      ...
      {!rows && !loading && (
        <p className="text-sm text-faint">Enter the PSC of what you\'re buying (e.g. J998 ship repair) to find every small business that\'s won that work.</p>
      )}
    </div>
  );
}
