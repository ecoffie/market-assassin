 ...
 *
 * Exposes Mindy's GovCon intelligence tools to ANY MCP-capable agent (Claude
 * Desktop, Cursor, a customer's own tool) — including `get_winning_playbook`, the
 * proprietary teaching corpus, which is the one Pro-gated tool on the surface.
 * (Pulled 2026-07-29, restored 2026-08-06 with the Pro entitlement check fixed.)
 *
...
server.registerTool(
  'get_winning_playbook',
  {
    ...
    description:
      ...
      'Mindy Pro only on the hosted surface — a non-Pro caller gets a clean ' +
      '`requires_pro` message and is NOT charged.',
    inputSchema: {
      topic: z
        .string()
        .min(3)
