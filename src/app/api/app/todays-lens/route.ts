...
export async function GET(request: NextRequest) {
  ...
  if (profErr) return NextResponse.json({ error: `profile read failed: ${profErr.message}` }, { status: 500 });
  ...
  // an error must 500, NEVER coalesce to 0 (count≠null: a broken count is UNKNOWN, not "no opps").
  ...
  if (totErr) return NextResponse.json({ error: `open-count failed: ${totErr.message}` }, { status: 500 });

  // Per-strand containment counts (opportunity_dna_keys @> ARRAY[strand]). One head-count per strand.
  const strands: { key: string; label: string; icon: string; count: number }[] = [];
  for (const s of LENS_STRANDS) {
    ...
    if (error) return NextResponse.json({ error: `strand '${s.key}' count failed: ${error.message}` }, { status: 500 });
  ...
  return NextResponse.json({
    ok: true,
    grounded,                          // false = 0 open opps in the user's codes (honest empty, NOT an error)
    usingFallback,                     // true = user had no NAICS → default codes (nudge them to set a profile)
    date: nowIso.slice(0, 10),
    totalOpen: totalOpen ?? 0,
    strands: present,                  // [{key,label,icon,count}] — the "why today matters" lines
    lensStrategy: lensStrategyKeys.join(','), // → /opportunity-map?strategy=<this> (the CTA target)
    note: grounded
