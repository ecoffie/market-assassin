...
// One consistent typeface for EVERY exported doc. Without an explicit default the
// docx library uses its built-in font while individual TextRuns set their own — so
// the document read as a patchwork (the LOI body in one font, the experience block
// in Courier New). Calibri 11pt (22 half-points) is the modern Word default; the
// heading styles inherit the same family. Applied to all four Document instances.
const DOC_STYLES = {
  default: {
...
// Twip position for the value column in the RELEVANT EXPERIENCE block (~2.4").
const EXPERIENCE_TAB = 3456;

// Render the deterministic LOI letter (a formatted plain-text string) as docx
// paragraphs. The whole letter stays in ONE font (DOC_STYLES default). The
// RELEVANT EXPERIENCE labels used to rely on space-padding + a Courier-New switch
// to line up the values — that font change was the formatting inconsistency. We
// now render each "Label:   value" line as a bold label + a TAB to a fixed stop,
// so the values align in the normal proportional font with no font switch.
function renderLoiTemplateParagraphs(letter: string): Paragraph[] {
  ...
  if (expParts.length) {
    out.push(new Paragraph({
      ...
      children: [new TextRun({ text: 'RELEVANT EXPERIENCE', bold: true })],
    }));
    const exp = expParts.join('RELEVANT EXPERIENCE');
    for (const raw of exp.split('\n')) {
      const line = raw.replace(/\s+$/, '');
      // "Label:   value" → bold label, tab, value (tab-aligned, single font).
      const labelled = line.match(/^([A-Za-z][A-Za-z /]*?):\s{2,}(.*)$/);
      if (labelled) {
        out.push(new Paragraph({
          tabStops: [{ type: TabStopType.LEFT, position: EXPERIENCE_TAB }],
          children: [
            new TextRun({ text: `${labelled[1]}:`, bold: true }),
            new TextRun({ text: `\t${labelled[2]}` }),
          ],
        }));
      } else {
        // Project name / blank separators — plain line in the same font.
        out.push(new Paragraph({ children: [new TextRun({ text: line })] }));
      }
    }
  }
...
function buildResponseTemplateSections(
  ...
  return [
    {
      label: isRfq ? 'Response Cover' : 'Letter of Intent',
      draft: [
        fill('Date', new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })),
        '',
        blank('Attention'),
...
export async function POST(request: NextRequest) {
    ...
    new Paragraph({
      ...
      children: [new TextRun({
        text: isSimpleResponsePackage
          ? isRfqPackage ? 'Response Template' : 'LOI Response Template'
          : 'Proposal Package',
        bold: true,
        size: 56,
      })],
  ...
  const doc = new Document({
    creator: 'Mindy',
    styles: DOC_STYLES,
    title: `${isSimpleResponsePackage ? isRfqPackage ? 'Response Template' : 'LOI Response Template' : 'Proposal Package'} — ${rfpName}`,
