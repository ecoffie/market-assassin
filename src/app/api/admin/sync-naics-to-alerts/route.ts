...
/**
 * RETIRED (410 Gone). This route synced NAICS into `user_alert_settings` — a table that
 * was DROPPED on purpose (memory: project_mindy_deadletter_automation — "sync-*-to-alerts
 * routes are DEAD — never re-schedule"). The real profile + alert config now lives entirely
 * in `user_notification_settings` and is written directly at save time. Do NOT re-point this
 * at the real table: that would revive the retired subsystem. Kept as a loud 410 so a stale
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    error: 'Gone: sync-naics-to-alerts is retired',
