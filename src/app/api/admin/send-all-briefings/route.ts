...
function generateDailyEmailHtmlFromSam(briefing: SamDailyBriefing, userEmail?: string): string {
  const actionBaseUrl = 'https://mi.govcongiants.com/api/actions/add-to-pipeline';
...
<body>
  <div class="container">
    ...
    <div class="footer">
      ...
      <p style="margin-top: 12px;"><a href="https://mi.govcongiants.com/alerts/preferences">Manage Preferences</a> | <a href="https://shop.govcongiants.com/briefings">View All Briefings</a></p>
    </div>
  </div>
</body>
</html>
...
function generateDailyEmailTextFromSam(briefing: SamDailyBriefing, userEmail?: string): string {
  ...
  for (const opp of briefing.opportunities) {
    ...
    if (userEmail) {
      text += `   → Not Interested: https://mi.govcongiants.com/api/actions/mute-opportunity?email=${encodeURIComponent(userEmail)}&title=${encodeURIComponent(opp.title)}&notice_id=${encodeURIComponent(opp.solicitationNumber || '')}\n`;
    }
    text += `\n`;
  }
      ...
      ${briefing.opportunities.map(opp => `
        <div class="opportunity">
          ...
          <div class="action-btns">
            <a href="https://mi.govcongiants.com/api/actions/mute-opportunity?email=${encodeURIComponent(userEmail)}&title=${encodeURIComponent(opp.contractName)}" class="action-btn btn-mute">🔇 Not Interested</a>
          </div>
          ` : ''}
        </div>
      `).join('')}
...
function generateDailyEmailText(briefing: DailyBriefing, userEmail?: string): string {
  ...
  for (const opp of briefing.opportunities) {
    ...
    if (userEmail) {
      text += `   → Not Interested: https://mi.govcongiants.com/api/actions/mute-opportunity?email=${encodeURIComponent(userEmail)}&title=${encodeURIComponent(opp.contractName)}\n`;
    }
    text += `${'─'.repeat(40)}\n\n`;
  }
