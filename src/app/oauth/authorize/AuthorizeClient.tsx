  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      setStage((current) => {
        ...
        setError('This authorization page took too long to load. Sign in to Mindy and try connecting again.');
        return 'error';
      });
    }, 8000);
    void (async () => {
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6 text-center">
        ...
        {stage === 'signin' && (
          <>
            ...
            <a
              href={mindySignUpUrl(typeof window !== 'undefined' ? `${window.location.pathname}${window.location.search}` : '/oauth/authorize')}
            ...
            <a
              href={mindySignInUrl(typeof window !== 'undefined' ? `${window.location.pathname}${window.location.search}` : '/oauth/authorize')}
