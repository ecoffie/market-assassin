...
import { connectionFlagsFromClients, type ConnectionFlags } from '@/lib/mcp/connection-state';
...
export default function McpConnectClient({ initialSession }: { initialSession: MindyCookieSession }) {
  const [authState, setAuthState] = useState<'in' | 'out' | 'error'>(
    initialSession.signedIn ? 'in' : 'out',
  );
  const [email, setEmail] = useState<string | null>(initialSession.signedIn ? initialSession.email : null);
  const [balance, setBalance] = useState<number | null>(null);
  const [copied, setCopied] = useState<string | null>(null);
  const [client, setClient] = useState<ClientId>('claude-desktop');
  const [catalog, setCatalog] = useState<Catalog | null>(null);
  const [connections, setConnections] = useState<ConnectionFlags>({
    claude: 'unknown',
    chatgpt: 'unknown',
    hasNamedClient: false,
  });

  // Cookie already decided identity for first paint. Client fetch can confirm
  // (header-compat) and pull balance / connection labels — it must not flash
  // a signed-out CTA over a server-signed-in session.
  useEffect(() => {
      ...
      try {
        ...
        if (res.ok && j?.email) {
          ...
          if (Array.isArray(j.connections)) {
            setConnections(connectionFlagsFromClients(j.connections));
          }
        } else if (!initialSession.signedIn) {
          setAuthState('out');
        }
      } catch {
        if (!initialSession.signedIn) setAuthState('error');
      }
    })();
    ...
    <main className="min-h-dvh bg-[#0a0f1e] text-slate-100 [color-scheme:dark]">
      <div className="mx-auto max-w-4xl px-5 py-8 sm:px-6">
        ...
        <section className="mt-12 text-center">
          ...
          {signedIn ? (
            <div className="mt-6 space-y-4">
              <p className="text-[13px] text-emerald-200">
                Mindy account recognized{email ? <> · <span className="text-slate-300">{email}</span></> : null}
              </p>
              <div className="flex flex-wrap items-center justify-center gap-3">
                <Link
                  href="/mcp/setup#claude"
                  className="inline-flex items-center justify-center rounded-xl bg-emerald-500 px-5 py-2.5 text-sm font-semibold text-[#06120c] hover:bg-emerald-400"
                >
                  {connections.claude === 'connected' ? 'Claude connected · Reconnect' : 'Connect Claude'}
                </Link>
                <Link
                  href="/mcp/setup#chatgpt"
                  className="inline-flex items-center justify-center rounded-xl border border-white/15 px-5 py-2.5 text-sm font-semibold text-slate-200 hover:bg-white/5"
                >
                  {connections.chatgpt === 'connected' ? 'ChatGPT connected · Reconnect' : 'Connect ChatGPT'}
                </Link>
              </div>
              <div className="flex flex-wrap items-center justify-center gap-3">
                <Link href="/mcp/account" className="text-[13px] text-slate-400 underline underline-offset-2 hover:text-slate-200">
                  {typeof balance === 'number' ? `${balance.toLocaleString()} credits · Account` : 'Account'}
                </Link>
                <Link href="/mcp/pricing" className="text-[13px] text-slate-400 underline underline-offset-2 hover:text-slate-200">See pricing</Link>
              </div>
            </div>
          ) : (
            <>
              <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
                <a href={mindySignInUrl('/mcp')} className="inline-flex items-center justify-center rounded-xl bg-emerald-500 px-5 py-2.5 text-sm font-semibold text-[#06120c] hover:bg-emerald-400">Sign in to Mindy</a>
          ...
          {authState === 'error' && (
            <p className="mt-3 text-[12px] text-amber-300">Could not refresh your session. Use Sign in to Mindy if this page looks wrong.</p>
          )}
        </section>
