...
async function exportPDF(result: DSBSScoreResult, benchmark: BenchmarkData | null) {
  ...
  doc.text('DSBS Profile Score Report', 20, y);
  y += 10;
  doc.setFontSize(10);
  doc.setTextColor(120, 120, 120);
  doc.text(`Generated ${new Date().toLocaleDateString()} | GovCon Giants AI`, 20, y);
  y += 15;

  // Overall Score
  doc.setFontSize(36);
  const scoreColor = result.overallScore >= 85 ? [34, 197, 94] : result.overallScore >= 70 ? [6, 182, 212] : result.overallScore >= 50 ? [245, 158, 11] : [239, 68, 68];
  doc.setTextColor(scoreColor[0], scoreColor[1], scoreColor[2]);
  doc.text(`${result.overallScore}/100`, 20, y);
  doc.setFontSize(14);
  doc.text(result.tierLabel, 75, y);
  y += 15;
  ...
  if (result.topRecommendations.length > 0) {
    ...
    doc.text('Top Priority Actions', 20, y);
    y += 8;
    doc.setFontSize(10);
  ...
  for (const section of result.sections) {
    ...
    doc.setFontSize(11);
    doc.setTextColor(0, 0, 0);
    doc.text(`${section.name}: ${section.score}/${section.maxScore} (${section.percentage}%)`, 22, y);
    y += 5;

    // Score bar
    doc.setFillColor(200, 200, 200);
    doc.rect(22, y, 100, 3, 'F');
    const barColor = section.percentage >= 80 ? [34, 197, 94] : section.percentage >= 60 ? [6, 182, 212] : section.percentage >= 40 ? [245, 158, 11] : [239, 68, 68];
    doc.setFillColor(barColor[0], barColor[1], barColor[2]);
    doc.rect(22, y, section.percentage, 3, 'F');
    y += 8;

    if (section.recommendations.length > 0) {
      doc.setFontSize(9);
      doc.setTextColor(100, 100, 100);
      for (const rec of section.recommendations.slice(0, 2)) {
  ...
  if (benchmark) {
    ...
    doc.setFontSize(14);
    doc.setTextColor(0, 0, 0);
    doc.text(`NAICS Benchmark: ${benchmark.naicsCode}`, 20, y);
    y += 8;
    doc.setFontSize(10);
    doc.setTextColor(60, 60, 60);
    doc.text(`${benchmark.totalContractors} contractors in your space`, 22, y); y += 6;
    doc.text(`Your percentile by contracts: ${benchmark.userPercentileByContracts}th`, 22, y); y += 6;
    doc.text(`Your percentile by value: ${benchmark.userPercentileByValue}th`, 22, y); y += 6;
    doc.text(`Avg contracts in NAICS: ${benchmark.avgContractCount}`, 22, y); y += 6;
    doc.text(`Median contract value: ${formatCurrency(benchmark.medianContractValue)}`, 22, y); y += 10;

    if (benchmark.topByValue.length > 0) {
      doc.setFontSize(11);
      doc.setTextColor(0, 0, 0);
      doc.text('Top Contractors by Value:', 22, y); y += 6;
      doc.setFontSize(9);
      doc.setTextColor(60, 60, 60);
      for (const c of benchmark.topByValue.slice(0, 5)) {
        doc.text(`${c.company} — ${c.contractCount} contracts, ${formatCurrency(c.totalValue)}`, 25, y);
        y += 5;
      }
    }
  }
  ...
  doc.text('Generated by GovCon Giants AI | tools.govcongiants.org/dsbs-scorer', 20, 285);

  doc.save(`dsbs-profile-score-${new Date().toISOString().slice(0, 10)}.pdf`);
}
      ...
      <div className="min-h-screen bg-slate-950 py-8 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <button onClick={() => exportPDF(result, benchmark)}
              className="px-6 py-3 bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 font-semibold rounded-lg transition-colors border border-blue-500/30">
              Download PDF Report
            </button>
