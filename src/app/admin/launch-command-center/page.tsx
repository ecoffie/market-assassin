...
function LoadingState() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-ground-deep px-6 text-white">
      <div className="w-full max-w-md rounded-lg border border-surface bg-ground p-8 shadow-2xl">
        ...
        <p className="mt-4 text-sm text-muted">Loading the internal launch workspace...</p>
        <style jsx>{`
          ...
          <form className="mt-8 space-y-4" onSubmit={handleLogin}>
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md px-3 py-1.5 text-sm font-semibold text-emerald-300 transition hover:bg-surface hover:text-emerald-200"
    ...
    <main className="min-h-screen bg-ground-deep text-white">
      <header className="border-b border-surface bg-ground-deep/95">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 px-6 py-8 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="rounded-lg border border-surface bg-ground px-5 py-4 text-right">
            ...
            <div className="mt-4 flex justify-end gap-2">
              <a className="rounded-lg border border-hairline px-3 py-2 text-sm text-ink-soft hover:border-slate-500" href="/command-center/dashboard">
                Admin Dashboard
              </a>
              <a className="rounded-lg border border-hairline px-3 py-2 text-sm text-ink-soft hover:border-slate-500" href="/command-center/accounts">
                Mindy Accounts
              </a>
              <button
                className="rounded-lg border border-hairline px-3 py-2 text-sm text-ink-soft hover:border-slate-500"
                onClick={() => {
                  sessionStorage.removeItem('adminAuth');
                  sessionStorage.removeItem('adminPassword');
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-6 py-8">
        ...
        <section className="rounded-2xl border border-emerald-500/30 bg-gradient-to-br from-emerald-900/25 via-slate-900 to-slate-900 p-6">
          <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex items-center gap-3">
              {(() => {
                ...
                return label ? (
                  <span className="text-xs text-faint" title="MRR reads a daily Stripe cache; this is its last sync. Click Refresh purchases to pull live.">
                    {label}
                  </span>
                ) : null;
              })()}
          ...
          {!mrrGoal && !mrrGoalError && <p className="mt-6 text-sm text-muted">Loading the goal…</p>}
          ...
          {mrrGoal && (
            <>
              ...
              <div className="mt-6 flex flex-wrap items-end gap-x-4 gap-y-1">
                ...
                <span className="text-xl text-muted">/ ${mrrGoal.goal.toLocaleString()} MRR</span>
              ...
              <div className="mt-7">
                ...
                <p className="text-xs text-faint">at three lead→paid close rates. IG peak was ~2,000 signups/mo — the Medium case is ~6 months at that pace.</p>
                    ...
                    return (
                      <div key={s.label} className={`rounded-xl border p-4 ${ring}`}>
                        <div className="flex items-baseline justify-between">
                          ...
                          <span className="text-xs text-faint">{Math.round(s.rate * 100)}% close</span>
                        </div>
                        <div className="mt-2 text-3xl font-black text-white">{s.leads.toLocaleString()}</div>
                        <div className="text-xs text-muted">total leads needed</div>
                        <div className="mt-2 text-sm text-ink-soft">≈ <b className={text}>{s.perMonth6.toLocaleString()}/mo</b> for 6 months</div>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="mt-6 rounded-xl border border-purple-500/30 bg-purple-500/5 p-4">
                  ...
                  <div className="mt-2 grid gap-3 sm:grid-cols-2">
                    {mrrGoal.lifetimeScenarios.map((l) => (
                      <div key={l.name} className="text-sm text-ink-soft">
                  ...
                  <p className="mt-2 text-xs text-faint">Plus Team ($499) & Enterprise tiers — every higher-tier sub counts as more than one $149.</p>
                </div>
              )}
              ...
              <div className="mt-6 rounded-xl border border-blue-500/30 bg-blue-500/5 p-4">
                ...
                {(mrrGoal.upgradeModalShown || 0) === 0 ? (
                  <p className="mt-1 text-xs text-faint">No upgrade-modal opens yet — free users haven&apos;t clicked a locked feature in this window (or it just shipped).</p>
                ) : (
                  <>
                    ...
                    {mrrGoal.topUpgradeFeatures && mrrGoal.topUpgradeFeatures.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-2">
                        {mrrGoal.topUpgradeFeatures.map((f) => (
                          <span key={f.feature} className="rounded-full border border-hairline bg-ground px-2.5 py-0.5 text-xs text-ink-soft">
                            {f.feature} × <b className="text-white">{f.count}</b>
                          </span>
                        ))}
                      </div>
                    )}
                    <p className="mt-2 text-xs text-faint">Which locked features drive the most upgrade intent — fuel for what to build/price next.</p>
                  </>
                )}
              </div>
              ...
              <div className="mt-6 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
                ...
                {upgradeIntentError ? (
                  ...
                  <p className="mt-2 text-xs text-faint">Loading upgrade clickers…</p>
                  ...
                  <>
                    <div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-sm text-ink-soft">
                      ...
                      {upgradeIntent.summary.alreadyPro > 0 ? (
                        <span><b className="text-muted">{upgradeIntent.summary.alreadyPro}</b> already Pro</span>
                      ) : null}
                    </div>
                    <div className="mt-3 max-h-72 space-y-2 overflow-y-auto">
                      {upgradeIntent.candidates
                        ...
                        .map((candidate) => (
                          <div key={candidate.email} className="rounded-lg border border-amber-500/20 bg-ground/60 p-3">
              ...
              {/* Current plan mix */}
              <div className="mt-5">
                <p className="text-xs uppercase tracking-wider text-faint mb-2">Current active plan mix ({mrrGoal.activeSubs} subs)</p>
                <div className="flex flex-wrap gap-2">
                  {mrrGoal.byPlan.filter((p) => p.monthlyPrice > 0).map((p) => (
                    <span key={p.monthlyPrice} className="rounded-full border border-hairline bg-ground px-2.5 py-0.5 text-xs text-ink-soft">
        ...
        <section className="rounded-lg border border-cyan-500/30 bg-gradient-to-br from-cyan-900/20 to-slate-900 p-6">
            ...
            {heatmap && (
              <div className="flex items-center gap-4">
                <div className="text-right">
                  ...
                  <div className="text-xs text-muted">collab-ready ({heatmap.totalTrackedOpps} tracked)</div>
                </div>
          ...
          {!heatmap && !heatmapError && <p className="mt-4 text-sm text-muted">Loading the heatmap…</p>}

          {heatmap && heatmap.opps.length === 0 && (
            <p className="mt-4 text-sm text-muted">No tracked opportunities yet.</p>
          )}
          ...
          {heatmap && heatmap.opps.length > 0 && heatmapOpen && (
            <div className="mt-5 overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left text-[11px] uppercase tracking-wider text-faint border-b border-hairline">
                ...
                <tbody className="divide-y divide-slate-800">
                  {heatmap.opps.slice(0, 20).map((o) => (
                    <tr key={o.noticeId} className="align-top hover:bg-surface/40">
                      <td className="py-2.5 pr-3 tabular-nums">
                        <span className="text-lg font-bold text-cyan-300">{o.trackerCount}</span>
                        {o.pursuingCount > 0 && <span className="text-[11px] text-faint"> · {o.pursuingCount} pursuing</span>}
                      </td>
                      <td className="py-2.5 pr-3 text-slate-200 max-w-md">
                        ...
                        {o.title}
                        {o.agency && <span className="block text-[11px] text-faint">{o.agency}</span>}
                      </td>
          ...
          {heatmap && heatmap.collabReadyCount > 0 && (
            <div className="mt-4 rounded-md border border-emerald-500/30 bg-emerald-900/15 p-3">
              ...
              {heatmap.opps.filter((o) => o.collabReady).slice(0, 3).map((o) => (
                <p key={o.noticeId} className="text-sm text-ink-soft mt-1">“{o.collabPreview}”</p>
              ))}
              <p className="text-[11px] text-faint mt-2">Phase 1: previews only — you control the first sends. Auto-trigger comes once volume proves the signal.</p>
            </div>
          )}
        </section>
        ...
        <section className="rounded-lg border border-violet-500/30 bg-gradient-to-br from-violet-900/20 to-slate-900 p-6">
          ...
          {!partnerBrief && !partnerBriefError && (
            <p className="mt-6 text-sm text-muted">Loading partner programs…</p>
          )}

          {partnerBrief && partnerBrief.programs.length > 0 && (
            <div className="mt-6 grid gap-4 lg:grid-cols-2">
              {partnerBrief.programs.map((p) => (
                <div key={p.code} className="rounded-xl border border-surface bg-ground-deep/50 p-5">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <p className="text-xs text-faint">Commission owed (lifetime)</p>
                      <p className="text-lg font-bold text-violet-300">{p.affiliate.commissionFormatted}</p>
                    </div>
                    <div>
                      <p className="text-xs text-faint">Paying customers</p>
                      <p className="text-lg font-bold text-white">{p.affiliate.payingCustomers}</p>
                    </div>
                    <div>
                      <p className="text-xs text-faint">Per $149/mo sub (affiliate / you)</p>
                    ...
                    <div>
                      <p className="text-xs text-faint">Est. monthly run-rate owed</p>
                      <p className="text-sm font-semibold text-slate-200">{p.affiliate.monthlyRunRateFormatted}</p>
                    </div>
                  </div>
                  <p className="mt-3 text-xs text-faint break-all">{p.urls.landing}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {partnerBrief && partnerBrief.programs.length === 0 && (
            <p className="mt-6 text-sm text-muted">No partner programs registered yet.</p>
          )}
        </section>
        ...
        <section className="rounded-lg border border-purple-500/30 bg-gradient-to-br from-purple-900/20 to-slate-900 p-6">
          ...
          {betaConvLoading && !betaConv && (
            <p className="mt-6 text-sm text-muted">Loading conversion funnel…</p>
          )}
          {betaConvError && (
          ...
          {betaConv && (
            <>
              <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-lg border border-surface bg-ground-deep/40 p-4">
                  <div className="text-3xl font-bold text-white">{betaConv.entitledTotal.toLocaleString()}</div>
                  <div className="mt-1 text-sm text-muted">Entitled beta users</div>
                </div>
                <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-4">
                  ...
                  <div className="mt-1 text-sm text-muted">Converted (have a login)</div>
                </div>
                <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4">
                  <div className="text-3xl font-bold text-amber-300">{betaConv.invitedPending.toLocaleString()}</div>
                  <div className="mt-1 text-sm text-muted">Invited, not set up yet</div>
                </div>
                <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4">
                  <div className="text-3xl font-bold text-blue-300">{betaConv.remaining.toLocaleString()}</div>
                  <div className="mt-1 text-sm text-muted">In send queue (~{betaConv.daysToDrain}d to drain)</div>
                </div>
              </div>
              ...
              {betaConv.sendTrend.length > 0 && (
                <div className="mt-5">
                  <p className="text-xs uppercase tracking-wider text-faint mb-2">Setup invites sent — last {betaConv.sendTrend.length} days</p>
        ...
        <section className="rounded-lg border border-teal-500/30 bg-gradient-to-br from-teal-900/20 to-slate-900 p-6">
          ...
          {trackAdoptionLoading && !trackAdoption && (
            <p className="mt-6 text-sm text-muted">Loading Track funnel…</p>
          )}
          {trackAdoptionError && (
            ...
            <>
              <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-lg border border-surface bg-ground-deep/40 p-4">
                  ...
                  <div className="mt-1 text-sm text-muted">Track clicks (14d)</div>
                </div>
                <div className="rounded-lg border border-surface bg-ground-deep/40 p-4">
                  <div className="text-3xl font-bold text-white">{trackAdoption.track_clicks.unique_users.toLocaleString()}</div>
                  <div className="mt-1 text-sm text-muted">Unique users clicking</div>
                </div>
                <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-4">
                  <div className="text-3xl font-bold text-emerald-300">
                  ...
                  <div className="mt-1 text-sm text-muted">Landed in pipeline</div>
                </div>
                <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4">
                  <div className="text-3xl font-bold text-blue-300">
                    {trackAdoption.track_clicks.by_affordance.trackButton}
                    <span className="mx-1 text-base text-faint">/</span>
                    <span className="text-blue-200">{trackAdoption.track_clicks.by_affordance.titleLink}</span>
                  </div>
                  <div className="mt-1 text-sm text-muted">📌 Button / title link</div>
                </div>
              </div>
        ...
        <section className="rounded-lg border border-surface bg-ground p-6">
          ...
          {insightEngLoading && !insightEng && (
            <p className="mt-6 text-sm text-muted">Loading Insight engagement…</p>
          )}
          {insightEngError && (
            ...
            <>
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-lg border border-surface bg-ground-deep/40 p-4">
                  ...
                  <div className="mt-1 text-sm text-muted">Impressions ({insightEng.impressions.unique_viewers.toLocaleString()} users)</div>
                </div>
                <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-4">
                  <div className="text-3xl font-bold text-emerald-300">
                  ...
                  <div className="mt-1 text-sm text-muted">Interactions ({insightEng.interactions.unique_users.toLocaleString()} users)</div>
                </div>
                <div className="rounded-lg border border-surface bg-ground-deep/40 p-4">
                  ...
                  <div className="mt-1 text-sm text-muted">↻ Refresh · ⎘ Copy</div>
                </div>
                <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4">
                  <div className="text-3xl font-bold text-amber-300">{insightEng.interactions.by_action.dismiss}</div>
                  <div className="mt-1 text-sm text-muted">× Dismissed</div>
                </div>
              </div>

              {insightEng.by_variant.length > 0 && (
                <div className="mt-5">
                  <div className="mb-2 text-xs uppercase tracking-wide text-faint">Which variant lands (impressions · interaction rate)</div>
                  <div className="space-y-1.5">
                    {insightEng.by_variant.map((v) => (
                      <div key={v.variant} className="flex items-center justify-between gap-3 rounded-md border border-surface bg-ground-deep/40 px-3 py-1.5 text-sm">
                        <span className="text-ink-soft">{v.variant}</span>
                        <span className="text-muted">
                          {v.impressions.toLocaleString()} seen
                          <span className="mx-1.5 text-slate-600">·</span>
                          <span className={v.interaction_rate_pct >= 5 ? 'text-emerald-300' : 'text-muted'}>{v.interaction_rate_pct}% acted</span>
                        </span>
                      </div>
                    ))}
                  </div>
        ...
        <section className="rounded-lg border border-surface bg-ground p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <button
              ...
              className="rounded-lg border border-hairline px-4 py-2 text-sm font-semibold text-slate-200 transition hover:border-emerald-400 hover:text-emerald-200"
            >
              Refresh Brief
            </button>
          </div>
          ...
          {growthLoading ? (
            <div className="mt-6 rounded-lg border border-surface bg-ground-deep/60 p-5">
              <div className="h-2 overflow-hidden rounded-full bg-surface">
                <div className="command-center-loader h-full w-1/3 rounded-full bg-gradient-to-r from-emerald-400 via-blue-400 to-purple-400" />
              </div>
              <p className="mt-3 text-sm text-muted">Loading MI growth signals...</p>
            </div>
          ) : growthError ? (
            ...
            <div className="mt-6 space-y-6">
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                  <p className="text-sm text-muted">Profiles Complete</p>
                  <p className="mt-2 text-4xl font-bold text-emerald-300">{formatNumber(growthBrief.audience.profileComplete)}</p>
                  <p className="mt-1 text-sm text-faint">{growthBrief.audience.profileCompletionRate} of {formatNumber(growthBrief.audience.activeAlertAudience)} active</p>
                </div>
                <div className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                  <p className="text-sm text-muted">Active In MI</p>
                  <p className="mt-2 text-4xl font-bold text-blue-300">{formatNumber(growthBrief.engagement.active7d)}</p>
                  <p className="mt-1 text-sm text-faint">{formatNumber(growthBrief.engagement.activeToday)} active today</p>
                </div>
                <div className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                  <p className="text-sm text-muted">Time In MI</p>
                  <p className="mt-2 text-4xl font-bold text-purple-300">{formatMinutes(growthBrief.engagement.timeInMiMinutes)}</p>
                  <p className="mt-1 text-sm text-faint">{formatMinutes(growthBrief.engagement.avgMinutesPerActiveUser)} per active user</p>
                </div>
                <div className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                  <p className="text-sm text-muted">Email Click Rate</p>
                  <p className="mt-2 text-4xl font-bold text-amber-300">{growthBrief.email.clickRate}</p>
                  <p className="mt-1 text-sm text-faint">{formatNumber(growthBrief.email.clicked)} clicks from {formatNumber(growthBrief.email.sent)} sent</p>
                </div>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-5">
                  <p className="text-sm text-muted">Paying / Pro</p>
                  <p className="mt-2 text-4xl font-bold text-emerald-300">{formatNumber(growthBrief.audience.miPro)}</p>
                  <p className="mt-1 text-sm text-faint">purchases + grants + lifetime/founders</p>
                </div>
                <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-5">
                  <p className="text-sm text-muted">Beta Trial</p>
                  <p className="mt-2 text-4xl font-bold text-amber-300">{formatNumber(growthBrief.audience.miTrial)}</p>
                  <p className="mt-1 text-sm text-faint">free beta_preview — should expire → convert</p>
                </div>
                <div className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                  <p className="text-sm text-muted">Free (alerts on)</p>
                  <p className="mt-2 text-4xl font-bold text-blue-300">{formatNumber(growthBrief.audience.miFree)}</p>
                  <p className="mt-1 text-sm text-faint">of {formatNumber(growthBrief.audience.activeAlertAudience)} active audience</p>
                </div>
              </div>
                ...
                <div className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    {(Object.keys(growthQueueLabels) as GrowthQueueName[]).map((queueName) => (
                      <div key={queueName} className="rounded-lg border border-surface bg-ground/70 p-4">
                        <p className="text-sm text-muted">{growthQueueLabels[queueName]}</p>
                        <p className="mt-2 text-3xl font-bold text-white">{formatNumber(growthBrief.queues[queueName]?.length || 0)}</p>
                        <p className="mt-1 text-xs text-faint">{growthBrief.queues[queueName]?.[0]?.owner || 'No owner needed'}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                  ...
                  <div className="mt-4 space-y-3">
                    {growthBrief.recommendedActions.slice(0, 4).map((item) => (
                      <div key={`${item.owner}-${item.action}`} className="rounded-lg border border-surface bg-ground/70 p-4">
                        <p className="text-sm font-semibold text-emerald-200">{item.owner}</p>
                        <p className="mt-1 text-sm text-white">{item.action}</p>
                        <p className="mt-1 text-xs text-faint">{item.why}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="mt-6 rounded-lg border border-surface bg-ground-deep/60 p-5 text-muted">
        ...
        <section className="rounded-lg border border-amber-500/30 bg-gradient-to-br from-amber-900/20 to-slate-900 p-6">
          ...
          {qualLoading ? (
            <div className="mt-6 rounded-lg border border-surface bg-ground-deep/60 p-5">
              ...
              <p className="mt-3 text-sm text-muted">Scoring customers by purchase history...</p>
            </div>
          ) : qualError ? (
            <div className="mt-6 rounded-lg border border-red-500/40 bg-red-500/10 p-5 text-red-100">
              {qualError}
            ...
            <div className="mt-6 space-y-6">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-4">
                  ...
                  <p className="mt-1 text-xs text-muted">Score 85+ • Founder calls</p>
                </div>
                <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-4">
                  ...
                  <p className="mt-1 text-xs text-muted">Incomplete profile (default NAICS) · score 30+ · setup nudges</p>
                </div>
                <div className="rounded-lg border border-red-500/30 bg-red-500/10 p-4">
                  ...
                  <p className="mt-1 text-xs text-muted">Paid but inactive • Re-engage</p>
                </div>
                <div className="rounded-lg border border-slate-500/30 bg-slate-500/10 p-4">
                  <p className="text-sm text-ink-soft">Total Purchasers</p>
                  <p className="mt-2 text-4xl font-bold text-white">{qualBrief.summary.uniquePurchasers}</p>
                  <p className="mt-1 text-xs text-muted">From {formatNumber(qualBrief.summary.totalPurchases)} purchases</p>
                </div>
              </div>

              {/* Founder Call Queue */}
              <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-lg border border-amber-500/20 bg-ground-deep/50 p-5">
                  ...
                  <p className="mt-1 text-sm text-muted">High-value customers worth Eric&apos;s time</p>
                  <div className="mt-4 max-h-80 space-y-3 overflow-y-auto">
                    {qualBrief.lists.founderCalls.slice(0, 8).map((candidate) => (
                      <div key={candidate.email} className="rounded-lg border border-hairline bg-ground/70 p-4">
                        ...
                        <p className="mt-2 text-xs text-muted">{candidate.why}</p>
                      </div>
                    ))}
                  </div>
                </div>

                <div className="rounded-lg border border-emerald-500/20 bg-ground-deep/50 p-5">
                  <div className="flex items-center justify-between">
                  ...
                  <p className="mt-1 text-sm text-muted">Customers ready for Branden&apos;s upgrade pitch</p>
                  <div className="mt-4 max-h-80 space-y-3 overflow-y-auto">
              ...
              <div className="rounded-lg border border-emerald-500/20 bg-ground-deep/50 p-5">
                ...
                <div className="mt-4 max-h-96 space-y-2 overflow-y-auto">
                  {(qualBrief.lists.activationCandidates || []).map((candidate) => (
                    <div key={candidate.email} className="rounded-lg border border-hairline bg-ground/70 p-3">
                      <div className="flex items-start justify-between gap-2">
                        <p className="font-mono text-sm text-white break-all">{candidate.email}</p>
                  ...
                  {(qualBrief.lists.activationCandidates?.length || 0) === 0 ? (
                    <p className="text-sm text-faint">No activation candidates in the top-50 list.</p>
                  ) : null}
                </div>
              </div>
              ...
              {qualBrief.lists.rescueCandidates.length > 0 && (
                <div className="rounded-lg border border-red-500/20 bg-ground-deep/50 p-5">
                  <div className="flex items-center justify-between">
                  ...
                  <p className="mt-1 text-sm text-muted">Paid customers who went dark — re-engage before churn</p>
        ...
        <section className="rounded-lg border border-surface bg-ground p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            {launchBrief ? (
              <p className="rounded-lg border border-surface bg-ground-deep/50 px-4 py-2 text-sm text-muted">
          ...
          {launchLoading ? (
            <div className="mt-6 rounded-lg border border-surface bg-ground-deep/60 p-5">
              ...
              <p className="mt-3 text-sm text-muted">Loading launch manager brief...</p>
            </div>
          ) : launchError ? (
            <div className="mt-6 rounded-lg border border-red-500/40 bg-red-500/10 p-5 text-red-100">
              {launchError}
            ...
            <div className="mt-6 space-y-6">
              <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-5">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <div className="rounded-lg bg-ground-deep/40 p-4">
                    <p className="text-xs uppercase tracking-[0.16em] text-muted">Public / SEO / Sales</p>
                    <p className="mt-1 font-mono text-sm text-emerald-200">{launchBrief.domainPolicy.publicSite}</p>
                  </div>
                  <div className="rounded-lg bg-ground-deep/40 p-4">
                    <p className="text-xs uppercase tracking-[0.16em] text-muted">Mindy Product Platform</p>
              ...
              <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
                {launchBrief.launches.map((launch) => (
                  <article key={launch.name} className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h3 className="text-xl font-bold text-white">{launch.name}</h3>
                        <p className="mt-1 text-sm text-faint">{launch.status}</p>
                      </div>
                    ...
                    <p className="mt-4 text-sm text-ink-soft">{launch.objective}</p>
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                      <div className="rounded-lg bg-ground/70 p-4">
                        <p className="text-xs uppercase tracking-[0.16em] text-faint">Blockers</p>
                        <p className="mt-2 text-2xl font-bold text-amber-200">{formatNumber(launch.blockers.length)}</p>
                      </div>
                      <div className="rounded-lg bg-ground/70 p-4">
                        <p className="text-xs uppercase tracking-[0.16em] text-faint">Done Signals</p>
                        <p className="mt-2 text-2xl font-bold text-emerald-200">{formatNumber(launch.changes.length)}</p>
                      </div>
                    </div>
                    {launch.blockers[0] ? (
                      <p className="mt-4 rounded-lg border border-surface bg-ground/70 p-3 text-xs text-muted">
                  ...
                  <div className="mt-4 space-y-3">
                    {launchBrief.ownerActions.slice(0, 8).map((item) => (
                      <div key={`${item.owner}-${item.action}`} className="rounded-lg border border-surface bg-ground/70 p-4">
                        ...
                        <p className="mt-2 text-xs text-faint">{item.why}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                  ...
                  <div className="mt-4 space-y-3">
                    {launchBrief.decisions.length > 0 ? launchBrief.decisions.slice(0, 6).map((item) => (
                      <div key={item.decisionNeeded} className="rounded-lg border border-surface bg-ground/70 p-4">
                        <p className="text-sm font-semibold text-purple-200">{item.owner}</p>
                        <p className="mt-1 text-sm text-white">{item.decisionNeeded}</p>
                        <p className="mt-2 text-xs text-faint">{item.whyItMatters}</p>
                      </div>
                    )) : (
                      <p className="rounded-lg border border-surface bg-ground/70 p-4 text-sm text-muted">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            {coachBrief && (
              <div className="text-right">
                ...
                <p className="text-xs text-faint">open coach activities</p>
              </div>
            )}
          </div>

          {coachLoading ? (
            <p className="mt-6 text-sm text-muted">Loading coach queue…</p>
            ...
            <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_1.1fr]">
              <div className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                ...
                <p className="mt-1 text-xs text-faint">Target: 20 outreach calls/week per coach (COACH-ENTERPRISE-BD-PLAN)</p>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <select
                    value={coachForm.coach}
                    onChange={(e) => setCoachForm((f) => ({ ...f, coach: e.target.value }))}
                    className="h-10 rounded-lg border border-hairline bg-surface px-3 text-sm text-white"
                  >
                ...
                <input
                  ...
                  className="mt-3 w-full h-10 rounded-lg border border-hairline bg-surface px-3 text-sm text-white placeholder-faint"
                />
                <input
                  ...
                  className="mt-2 w-full h-10 rounded-lg border border-hairline bg-surface px-3 text-sm text-white placeholder-faint"
                />
                <button
                  type="button"
                  onClick={logCoachActivity}
                  disabled={!coachForm.target_org.trim()}
                  className="mt-3 h-10 w-full rounded-lg bg-blue-600 px-4 text-sm font-medium text-white hover:bg-blue-500 disabled:bg-input"
                >
                  Log activity
                </button>
                {coachLogStatus && (
              ...
              <div className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                ...
                <div className="mt-4 max-h-72 overflow-y-auto space-y-2">
                  {(coachBrief?.activities || []).length === 0 ? (
                    <p className="text-sm text-faint">No activities logged yet. Coaches: log your first APEX/SBDC outreach call above.</p>
                  ) : (
                    coachBrief!.activities.slice(0, 12).map((row) => (
                      <div key={row.id} className="rounded-lg border border-surface bg-ground/70 p-3">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-sm font-semibold text-blue-200">{row.coach}</span>
                          <span className="text-[10px] uppercase tracking-wider text-faint">{row.status}</span>
                        </div>
                        <p className="mt-1 text-sm text-white">{row.target_org || row.target_name || '—'}</p>
                        <p className="text-xs text-faint">{row.activity_type.replace(/_/g, ' ')}</p>
                      </div>
                    ))
                  )}
                </div>
        ...
        <section className="rounded-lg border border-surface bg-ground p-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.2em] text-faint">Operating Lanes</p>
              <h2 className="mt-2 text-3xl font-bold">Who owns what</h2>
              <p className="mt-1 text-xs font-semibold uppercase tracking-wide text-faint">Manual org snapshot · owners as of {OPS_SNAPSHOT_AS_OF}</p>
            </div>
            ...
            {roleLanes.map((lane) => (
              <article key={lane.name} className={`rounded-lg border p-5 ${toneClasses[lane.tone]}`}>
                ...
                <div className="mt-5 grid gap-3 md:grid-cols-2">
                  <div className="rounded-lg bg-ground-deep/40 p-4">
                    <p className="text-xs uppercase tracking-[0.16em] text-muted">Next action</p>
                    <p className="mt-2 text-sm text-slate-100">{lane.nextAction}</p>
                  </div>
                  <div className="rounded-lg bg-ground-deep/40 p-4">
                    <p className="text-xs uppercase tracking-[0.16em] text-muted">Signal to capture</p>
        ...
        <section className="grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-lg border border-surface bg-ground p-6">
            <p className="text-sm uppercase tracking-[0.2em] text-faint">This Week</p>
            ...
            <div className="mt-6 overflow-hidden rounded-lg border border-surface">
              <table className="w-full min-w-[760px] border-collapse text-left text-sm">
                <thead className="bg-ground-deep text-xs uppercase tracking-[0.16em] text-faint">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-800">
                  {priorities.map((item) => (
                    <tr key={item.priority} className="bg-ground/80">
                      <td className="px-4 py-4 font-semibold text-white">{item.priority}</td>
                      <td className="px-4 py-4 text-ink-soft">{item.owner}</td>
                      <td className="px-4 py-4 text-ink-soft">{item.decision}</td>
          ...
          <div className="rounded-lg border border-surface bg-ground p-6">
            <p className="text-sm uppercase tracking-[0.2em] text-faint">Metrics Model</p>
            <h2 className="mt-2 text-3xl font-bold">What we need to track</h2>
            <div className="mt-6 space-y-3">
              {metrics.map(([label, detail]) => (
                <div key={label} className="rounded-lg border border-surface bg-ground-deep/50 p-4">
        ...
        <section className="rounded-lg border border-surface bg-ground p-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.2em] text-faint">Operating Queues</p>
              <h2 className="mt-2 text-3xl font-bold">Team work that should become data</h2>
              <p className="mt-1 text-xs font-semibold uppercase tracking-wide text-faint">Manual snapshot · queue statuses as of {OPS_SNAPSHOT_AS_OF} (not live)</p>
            </div>
            <p className="max-w-2xl text-sm text-muted">
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-2">
            {queueItems.map((item) => (
              <article key={`${item.queue}-${item.owner}`} className="rounded-lg border border-surface bg-ground-deep/50 p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                ...
                <p className="mt-4 text-sm font-semibold text-ink-soft">{item.segment}</p>
                <p className="mt-2 text-sm text-muted">{item.nextAction}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="grid gap-8 lg:grid-cols-2">
          <div className="rounded-lg border border-surface bg-ground p-6">
            <p className="text-sm uppercase tracking-[0.2em] text-faint">Source of Truth</p>
            <h2 className="mt-2 text-3xl font-bold">Planning docs</h2>
            <div className="mt-6 space-y-3">
              {sourceDocs.map((doc) => (
                <div key={doc} className="rounded-lg border border-surface bg-ground-deep/50 px-4 py-3 font-mono text-sm text-ink-soft">
                  {doc}
                </div>
              ))}
            </div>
          ...
          <div className="rounded-lg border border-surface bg-ground p-6">
            <p className="text-sm uppercase tracking-[0.2em] text-faint">Next Build Slice</p>
            <h2 className="mt-2 text-3xl font-bold">Make it operational</h2>
            <div className="mt-6 space-y-3 text-sm text-ink-soft">
              <p className="rounded-lg border border-surface bg-ground-deep/50 p-4">
