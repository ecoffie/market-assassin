/**
 * /home — the LOGGED-IN home (GOS Decisions #010-#012): MCP-first command center.
 * The agent is the hero (connect + Ideas Gallery); your profile/tracking live here; real
 * gamification (XP/streak/anonymized leaderboard) from user_engagement. The app is a
 * de-emphasized fallback card. Shares the dark look with the public landing (PR #385).
 *
 * Preview: pass ?email=<user> to render that user's real stats. Not yet wired as the
 * post-signin destination — this is the preview build for sign-off.
 */
...
export default async function LoggedInHome({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [stats, board, opps] = await Promise.all([
    ...
    queryExpiringContracts({ monthsWindow: 12, minValue: 10_000_000, limit: 8 })
      .then((r) => r.contracts.slice(-3)).catch(() => [] as ExpiringContract[]),
  ]);
      ...
      <main className="wrap" id="top">
        <div className="hello">
          <div>
            ...
            <h1 className="disp">Good to see you{stats ? `, ${stats.codename}` : ''}.</h1>
            <p>Talk to your agent, put Mindy to work, and climb the board. Here&apos;s what&apos;s moving today.</p>
          </div>
        </div>

        {/* HERO — MCP-first: connect the agent (left) + today's intel (right) */}
        <section className="hero">
          <div className="card connect">
            <div className="grid-tex" />
            <div className="eyebrow" style={{ color: '#d8b4fe' }}>Mindy MCP · the agent</div>
            <h2 className="disp">The analyst you<br />just talk to.</h2>
            <div className="sub">No new tool to learn. Connect Mindy to the AI agent you already use — then ask it, in plain English, to find contracts, size markets, and draft proposals. A kid could do it.</div>
            <div className="cta">
              <Link className="btn-primary" href="/mcp">Connect your agent →</Link>
              <a className="btn-ghost" href="#ideas">See what to ask ↓</a>
            </div>
          </div>

          <div className="card today">
            <div className="hd"><div className="eyebrow">Today on Mindy</div><div className="live"><span className="dot" />Live</div></div>
            {opps.length === 0 ? <div className="muted">Updating…</div> : opps.map((c) => {
              const d = c.period_of_performance_current_end ? Math.max(0, Math.round((new Date(c.period_of_performance_current_end).getTime() - Date.now()) / 864e5)) : null;
              return (
