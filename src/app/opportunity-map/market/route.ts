  ...
  fetch('/api/app/market-report',{method:'POST',headers:{'Content-Type':'application/json','x-mi-auth-token':t,'x-user-email':em},body:JSON.stringify(payload)})
    ...
    .then(function(res){
      ...
      if(res.status===401){ if(typeof window.__mapsSignIn==='function'){window.__mapsSignIn();}
        rptErr('Your session expired \u2014 sign in to continue.'); return; }
...
${ACCOUNT_MENU_JS}
${LOGIN_MODAL_HTML}${LOGIN_MODAL_JS}${'<script>'}window.__mapsSignIn=function(){var stale=false;try{stale=!!localStorage.getItem('mi_beta_email')&&!localStorage.getItem('mi_beta_auth_token');}catch(e){}var phrase=stale?'continue where you left off':'run the market report for this search';if(typeof window.openSignInModal==='function'){window.openSignInModal(phrase,function(){location.reload();});}else{location.href='/signin?next='+encodeURIComponent('/opportunity-map/market');}return false;};${'</script>'}</body></html>`;
