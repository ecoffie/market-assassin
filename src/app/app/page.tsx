...
function AppDashboard() {
  ...
  const handleAppleSignIn = useCallback(async () => {
    ...
    if (!result.success) {
      setAuthError(result.error || 'Could not connect with Apple');
      setOauthLoading(null);
    }
  }, []);
  const handleGitHubSignIn = useCallback(async () => {
    ...
    if (!result.success) {
      setAuthError(result.error || 'Could not connect with GitHub');
      setOauthLoading(null);
    }
  }, []);
              ...
              <>
                <div className="space-y-3 mb-4">
                  ...
                  <button
                    ...
                    disabled={oauthLoading !== null || authLoading}
                    className="w-full inline-flex items-center justify-center gap-3 px-4 py-3 bg-black hover:bg-black/80 text-white rounded-lg font-medium text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed border border-hairline"
                  >
                    {oauthLoading === 'apple' ? (
                      <span className="w-4 h-4 border-2 border-slate-400 border-t-transparent rounded-full animate-spin" />
                    ) : (
