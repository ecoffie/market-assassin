...
export async function getUserProducts(email: string): Promise<{
  ...
  const productInfo: Partial<Record<ProductId, { name: string; url: string }>> = {
    'market-assassin': { name: 'Federal Market Assassin', url: '/app?panel=research' },
    ...
    'opportunity-hunter-pro': { name: 'Opportunity Hunter Pro', url: '/app?panel=research' },
    'recompete': { name: 'Recompete Tracker', url: '/app?panel=recompetes' },
