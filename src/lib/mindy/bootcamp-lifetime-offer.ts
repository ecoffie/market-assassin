...
export function bootcampLifetimeSubject(): string {
  return 'Your bootcamp alumni rate on Founders Lifetime (closes soon)';
}
...
<p><b>Founders Lifetime is $${FOUNDERS_LIFETIME_PRICE.toLocaleString()}</b> — the same lifetime price people paid for our courses before Mindy existed, and what we're selling publicly (100 founding seats). Because you attended bootcamp, you get an alumni window at <b>$${BOOTCAMP_LIFETIME_PRICE.toLocaleString()}</b> through ${deadline}:</p>
<div style="margin:20px 0;padding:20px;border:1px solid #7c3aed;border-radius:14px;background:linear-gradient(135deg,#1e3a8a0d,#7c3aed14);">
  <p style="margin:0 0 6px;font-weight:700;font-size:18px;color:#111827;">Founders Lifetime — Bootcamp Alumni Rate</p>
...
<p style="color:#6b7280;font-size:14px;margin-top:24px;">P.S. This email is for bootcamp attendees only — we don't advertise the alumni rate on the site. If lifetime isn't the move right now, Mindy is still free for daily alerts.</p>
`;
