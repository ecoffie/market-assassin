...
function readControlJson(script: string): unknown | null {
  try {
    // Turbopack treats join(cwd, 'scripts', x) as a dynamic import of
    // ./ROOT/scripts and fails `next build` (2026-09-13 prod deploys).
    const cwd = process.cwd();
    const file = `${cwd}${cwd.endsWith('/') ? '' : '/'}scripts/${script}`;
    const out = execFileSync(process.execPath, [file, '--json'], {
