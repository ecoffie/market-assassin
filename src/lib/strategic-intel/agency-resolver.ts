 ...
 *
 * Real callers do NOT pass canonical names. `src/lib/opportunities/opp-intel.ts`
 * passes `normalizeAgencyKey(agency)`, which STRIPS "DEPARTMENT"/"OF"/"THE"/
 * "AGENCY"/"ADMINISTRATION" — so the Opportunity Map's "Know your buyer" drawer
 * asks for "VETERANS AFFAIRS", not "Department of Veterans Affairs", and
 * "ENVIRONMENTAL PROTECTION", not "Environmental Protection Agency".
 *
 * This index maps each canonical name's OWN normalized form back to it, so those
 * callers resolve on a WHOLE-STRING key — never a substring. Built from the
 * canonical list itself, so it cannot mint an agency that does not exist.
 */
const BY_NORMALIZED = new Map<string, string>();
{
  const strip = (s: string) => norm(s)
    .replace(/[.,]/g, ' ')
    .split(' ')
    .filter((w) => w && !['DEPARTMENT', 'DEPT', 'OF', 'THE', 'US', 'U.S.', 'UNITED', 'STATES',
      'AGENCY', 'ADMINISTRATION', 'NATIONAL', 'OFFICE'].includes(w))
    .join(' ')
    .trim();
  for (const name of Object.keys(CODES)) {
    const k = strip(name);
    // Only index a key that is NOT itself generic and is unambiguous.
    if (!k || GENERIC_TOKENS.has(k)) continue;
    if (BY_NORMALIZED.has(k) && BY_NORMALIZED.get(k) !== name) { BY_NORMALIZED.set(k, ''); continue; }
    BY_NORMALIZED.set(k, name);
  }
  for (const [k, v] of [...BY_NORMALIZED]) if (!v) BY_NORMALIZED.delete(k); // drop ambiguous
}
...
export function resolveAgency(input: ResolveAgencyInput): AgencyResolution {
  ...
  // ── 4b. NORMALIZED WHOLE-STRING KEY (what normalizeAgencyKey callers pass) ─
  const viaNormalized = BY_NORMALIZED.get(key);
  if (viaNormalized) {
    return {
      canonicalAgency: viaNormalized,
      toptierCode: CODES[viaNormalized]?.code ?? null,
      subAgency, officeCode,
      method: 'alias',
      confidence: 'high',
      resolved: true,
      input: label,
      note: `Normalized agency key "${label}" → ${viaNormalized}.`,
    };
  }
