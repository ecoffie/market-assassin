...
export async function sendMarketIntelligenceWelcomeEmail({
  ...
  return sendEmail({
    to,
    subject: 'Your Mindy AI Access is Ready | GovCon Giants',
    ...
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #111827; max-width: 620px; margin: 0 auto; padding: 24px;">
        <h1 style="color: #1e3a8a; margin-bottom: 8px;">Your Mindy AI access is ready</h1>
        <p>Hi${customerName ? ` ${customerName}` : ''},</p>
    ...
    `,
    text: `Your Mindy AI access is ready.
...
export async function sendAlertProWelcomeEmail({
  ...
  // Market Assassin is retired as a standalone product. This block is an UPSELL (Alert Pro
  // is not a Pro grant in verifyMIAccess), so it points at the Mindy sales page rather than
  // the retired `/market-assassin`. Paid welcome/access emails link /app directly.
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.5; color: #1f2937; max-width: 600px; margin: 0 auto; padding: 20px; background: #f8fafc;">
  ...
  <div style="background: #ffffff; padding: 28px; border: 1px solid #e2e8f0; border-top: none;">
    ...
    <div style="background: linear-gradient(135deg, #fef2f2 0%, #fee2e2 100%); border: 1px solid #fca5a5; border-radius: 8px; padding: 18px; margin: 24px 0;">
      <h4 style="color: #991b1b; margin: 0 0 10px 0;">🎯 Ready to Win? Try Mindy</h4>
      <p style="color: #7f1d1d; margin: 0 0 12px 0; font-size: 14px;">Finding opportunities is step one. Mindy shows you how to win them with agency intelligence, competitor analysis, and market research.</p>
      <a href="${maLink}" style="background: #dc2626; color: white; padding: 10px 18px; text-decoration: none; border-radius: 6px; font-weight: 600; display: inline-block; font-size: 14px;">Explore Mindy →</a>
    </div>
