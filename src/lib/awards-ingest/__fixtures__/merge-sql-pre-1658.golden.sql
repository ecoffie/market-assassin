      ...
      MERGE `market-assasin.usaspending.awards` T
      USING (
        SELECT
        ...
        FROM `market-assasin.usaspending.awards_ingest_staging`
        WHERE contract_transaction_unique_key IS NOT NULL
          AND contract_transaction_unique_key != ''
      ) S
      ON T.txn_id = S.txn_id AND T.action_date >= DATE_SUB(DATE('2026-06-07'), INTERVAL 2 DAY)
