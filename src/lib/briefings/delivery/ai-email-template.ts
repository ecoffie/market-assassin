 ...
 */
export function generateAIEmailTemplate(briefing: AIGeneratedBriefing, userEmail?: string, trackingToken?: string): AIEmailTemplate {
  ...
  return {
    subject,
    preheader,
    htmlBody: generateHtmlBody(briefing, date, userEmail, trackingToken),
 ...
 */
function generateHtmlBody(briefing: AIGeneratedBriefing, date: string, userEmail?: string, trackingToken?: string): string {
  ...
  const feedbackParams = userEmail
    ...
    : '';
  const helpfulUrlRaw = feedbackParams ? `${feedbackBaseUrl}${feedbackParams}&rating=helpful` : '';
  const notHelpfulUrlRaw = feedbackParams ? `${feedbackBaseUrl}${feedbackParams}&rating=not_helpful` : '';
  // Wrap feedback URLs with click tracking if token available
  const helpfulUrl = helpfulUrlRaw && trackingToken ? generateTrackedLink(trackingToken, helpfulUrlRaw, 'feedback_helpful') : helpfulUrlRaw;
  const notHelpfulUrl = notHelpfulUrlRaw && trackingToken ? generateTrackedLink(trackingToken, notHelpfulUrlRaw, 'feedback_not_helpful') : notHelpfulUrlRaw;
  return `
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <div class="container">
    ...
    <div style="background: linear-gradient(90deg, #7c3aed 0%, #a855f7 100%); padding: 12px 20px; text-align: center;">
      <p style="color: white; margin: 0; font-size: 13px; font-weight: 600;">
        📊 Your weekly recompete analysis is ready! Here's what Mindy found.
      </p>
    </div>
    ...
    <div class="footer">
      <p>
        ...
        <a href="${trackingToken ? generateTrackedLink(trackingToken, 'https://mi.govcongiants.com/briefings', 'view_dashboard') : 'https://mi.govcongiants.com/briefings'}">View in dashboard</a> |
