...
|---|-----|-------------|-------|--------------------|--------|
| 1 | **DIBBS** (~3.3M DLA small-buy NSN/parts RFQs) | ✅ FEASIBLE — scraped via US residential proxy (Apify `parseforge/dibbs-rfq-scraper`, $7/1k). Our own probe failed on the WAF; proxy is the workaround. EULA: Eric said move forward (Jul 1). | Med-High: NSN/parts + historical "what price wins" pricing. Helps supplier/manufacturer users. | When (a) supplier/manufacturer users ask, OR (b) the DLA NV012 SBIR makes DLA data strategic. | ✅ **LIVE (Jul 1 2026)** — migration+cron+prod token all verified; sidebar DIBBS panel + `/api/app/dibbs` search shipped (`fa487799`). sync-dibbs cron daily 08:00 UTC. |
