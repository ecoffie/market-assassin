...
**Corpus as of 2026-08-02: 33,075 forecasts · 21 agencies · 388 buying offices ·
19,014 with a POC email · 18,686 mappable.** The per-source counts below are the
rows that survived ingest + dedupe, so they will not always sum to the total.
...
| **NASA** | 225 | `hq.nasa.gov/office/procurement/forecast/NAF.html` | Quarterly (Oct + Apr) | 14-column grid rendered client-side, **no JSON endpoint** — scrape the table, click "Show All", then walk pagination (50/page × 3). Check the per-center counts in the page's own filter sidebar sum to the scraped total. |
