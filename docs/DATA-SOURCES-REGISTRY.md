...
| **Mindy Institute — Federal Legislation** | Legislative evidence corpus (NDAA + future approps), provenance for legislative claims | `institute_sources` table ← **api.congress.gov** via `/api/cron/institute-legislation-sync`. **Discovery is DYNAMIC** — pages `/bill/{congress}` by `updateDate` and matches on TITLE, so a new fiscal year's bill is found with no code change; **no bill number, fiscal year, or Congress is hardcoded**. **ONE ROW PER LEGISLATIVE VERSION** (House introduced/reported/engrossed · Senate versions · committee reports · conference text · enacted law) — never collapsed into one "NDAA" record. Authenticated with the existing api.data.gov key. | **Weekly** (cron) | rolling (`data_sources.institute_legislation`) |
