...
| **Mindy Institute — Federal Legislation** | Legislative evidence corpus (NDAA + future approps), provenance for legislative claims | `institute_sources` table ← **api.congress.gov** via `/api/cron/institute-legislation-sync`. **Discovery is DYNAMIC and WATERMARKED** — two separate jobs: (A) *discovery* scans `/bill/{congress}` over a `fromDateTime`-bounded window and proves coverage by comparing rows read against the API's own `pagination.count` (a page ceiling yields `partial`, and the cursor does **not** advance); (B) *tracking* re-polls every measure already in the corpus **by identity**, so a known bill stays tracked forever even after thousands of unrelated bills push it out of the recent-update window. **No bill number, fiscal year, or Congress is hardcoded**; a Congress change discards the cursor and forces a full pass. **ONE ROW PER LEGISLATIVE VERSION** (House introduced/reported/engrossed · Senate versions · committee reports · conference text · enacted law) — never collapsed into one "NDAA" record. ⛔ **PARKED / BLOCKED_CONTROLLED 2026-09-21** — `source_state=upstream_quiet`, `intervention_state=blocked`, `manual_action_type=credential_renewal`: `CONGRESS_API_KEY` is ABSENT in production (the collector runs on the `GOVINFO_API_KEY` fallback) and the 29 held rows reach **zero** customer surfaces. The weekly cron stays enabled but never writes `data_source_instances`, so no run can stamp a data clock while parked. Unblock = provision the key **and** wire the corpus to a surface. See [the runbook](runbooks/institute-legislation.md). | **Weekly** (cron) | rolling (`data_sources.institute_legislation`) |
