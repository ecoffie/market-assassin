...
| **SBIR/STTR** | MCP `search_sbir` — NIH RePORTER awarded projects + live DoD DSIP Open/Pre-Release topics (`source="dod"`) + multisite open notices | `api.reporter.nih.gov/v2/projects/search` (activity codes R43/R44/R41/R42) + `www.dodsbirsttr.mil/topics/api/public/topics` (`src/lib/sbir/dsip.ts`) + Supabase `aggregated_opportunities` (`src/lib/sbir/search.ts`) | Official NIH RePORTER (source="nih" = AWARDED projects, not open sols). source="dod" is live DSIP topics (topic codes + close dates) — a degraded empty is a feed failure, not "no DoD SBIR exists". sbir.gov public API is 403 as of 2026-09-08; the `dod_sbir_topics` cache is last-good only. NIH is health-research heavy. |
