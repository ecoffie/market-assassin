...
| **Compliance scanner (deterministic)** | MCP `scan_proposal_compliance` — pre-submit DQ-risk scan (deadline, page limits, set-aside eligibility, reps/certs, unaddressed eval factors) → severity dq/warning/info + at_risk | `src/lib/proposal/compliance-scanner.ts` (PURE, no LLM/IO) — runs entirely on the requirements + draft the caller passes | Not a data source — a deterministic pattern check on caller-supplied inputs. Only as complete as the requirements passed in (pair with an extraction step for full coverage). NOT a substitute for human compliance review of substance. |
| **Bid/no-bid framework** | MCP `evaluate_bid_decision` — GovCon Giants' 5 eliminator gates + 10-factor scorecard; scores pursue/watch/skip/no-bid when the caller supplies gate answers + ratings | `src/lib/proposal/bid-decision.ts` (PURE, no LLM/IO) — Eric's framework verbatim | Not a data source — scores a bidder's SELF-ASSESSMENT (judgment calls, not measured facts). A failed gate is an absolute No-Bid regardless of score. Framework always returns even with no input. |
