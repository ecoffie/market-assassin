...
| **SBIR/STTR** | ⛔ MCP `search_sbir` **RETIRED 2026-09-26** (no reliable open-topic source; stale calls answered `tool_retired`, 0 credits — `src/lib/mcp/retired-tools.ts`). Libraries + stored data kept: `src/lib/sbir/*`, the in-app `/api/sbir` panel, `aggregated_opportunities`, `dod_sbir_topics`. | `api.reporter.nih.gov/v2/projects/search` + Supabase `aggregated_opportunities` (`src/lib/sbir/search.ts`) | Record: `tasks/sbir-reed-investigation-2026-09-26.md` (PR #1708). Specialty feeds remain PARKED. |
