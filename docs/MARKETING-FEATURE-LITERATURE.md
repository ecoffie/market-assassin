...
## Opportunity Map pin popup redesigned to lead with buyer identity, not the price (2026-08-03)

**What:** Clicking a pin on the Opportunity Map opens a popup card that now leads with the **buyer's identity** — the specific sub-agency (e.g. "EPA Region 4", "Veterans Health Administration") in small-caps typography with a calm landmark anchor and, when the buyer is genuinely small-business-friendly, an "SB-friendly" badge. The **value comes second**: a recompete (an awarded contract) shows its **real firm contract value** ("$46.9M") as a hard number, while an open opportunity (no award yet) shows an estimate band with a "≈" mark so it reads as an estimate, never a quoted price. The redundant "Current incumbent" line — which on a recompete just repeated the company already named at the top of the card — is hidden unless there's a genuinely different incumbent to show. The popup now speaks the exact same visual language as the result-list card, so the pin, the card, and the popup can never disagree.

**Why:** The old popup led with the company title and a facts grid that put "Contract value" first, so every pin looked the same at a glance and the number competed with the name for attention. Leading with the sub-agency gives each opportunity an identity a contractor recognizes ("this is an Army buy", "this is a VA buy") — the way a listing leads with the neighborhood, not the price. Showing the real contract value on a recompete (with no "≈") and a clearly-marked estimate on an open opportunity keeps the number honest: a firm award figure and a modeled estimate are different things and should never look identical. Dropping the incumbent-equals-title line removes noise so the card says one thing once.

**SEO:** federal opportunity map, contract recompete value, incumbent contractor lookup, sub-agency buyer intelligence, GovCon opportunity card, place of performance map.

**Proof:** Rendered headless from the real popup code and screenshot-verified before shipping — a KEMRON recompete showed the EPA Region 4 identity above a firm "$46.9M" (no "≈") with the incumbent line correctly absent (the company is the title), and an open Cloud Migration opportunity showed the Veterans Health Administration identity above an "≈ $4.9M" estimate with a genuinely different incumbent (Booz Allen Hamilton) shown. Grounded in real data: the value on a recompete is the actual award amount (never a fabricated "$0" — a recompete with no value on file says so honestly), and the "SB-friendly" badge reflects the buyer's measured purchase-order share, computed server-side, not an AI guess. 11 unit tests lock the identity-first order, the real-value-no-"≈" rule, and the incumbent-hidden-when-equal rule.

---

## Opportunity Map pins now cluster at zoom-out so the map is readable, not a wall of price tags (2026-08-03)

**What:** When you zoom the Opportunity Map out to a country or multi-state view, nearby pins now collapse into a single **count bubble** — e.g. `40 Opportunities · $311M` — instead of a wall of overlapping value tags. Zoom back in and the bubbles expand into the individual `$`-value pins, exactly as before. Click a bubble and the map flies in toward that cluster and opens it up. The same behavior powers the **Network** map, where a bubble reads `23 Contractors · 7 Agencies`. It's the Google-Maps / Zillow model: one legible count at a distance, individual listings up close.

**Why:** At country zoom the eastern US was an unreadable pile of overlapping price tags — you couldn't tell where the density was or read a single number. Clustering turns that noise into signal: a bubble tells you *how many* opportunities sit in a region and *how much* total value is on the table there, so you can spot the hot corridors at a glance and drill into the one that matters. The count and dollar total on each bubble are real — the dollars are summed from the individual opportunities' own estimates and contract values, so a cluster's total always equals the sum of the pins inside it. Nothing is invented: a bubble sits at the true average location of its members, and a region with no dollar figures shows just the count.

**SEO:** federal opportunity map, contract opportunity clustering, opportunity density map, GovCon map by region, contract value heat map, opportunities by state.

**Proof:** Verified headless with the real Leaflet map before shipping — a 72-pin eastern-US view collapsed into 7 readable bubbles (`40 Opportunities · $311M`, `10 Opportunities · $146M`, …) at country zoom, and at street zoom the same rows rendered as 72 individual value pins with zero cluster bubbles (before/after screenshots). Grounded in real data: the dollar total sums each opportunity's actual estimate or contract value (never a formatted-string parse that would undercount), the count is a real member count, and the bubble's position is the members' true centroid. The Network map bubble showed `23 Contractors · 7 Agencies` and drops any zero segment (never "0 Agencies"). 16 unit tests guard the bucketing, the entity-aware labels, the zoom threshold, the real-sum, and the drill-in click.

---

## The M-Estimate now prices what was *bought*, not the seller's industry code — and never invents a giant (2026-08-03)

**What:** Every opportunity on the map carries an **M-Estimate™** — a grounded dollar range for what the work is likely worth. This release makes that number trustworthy in every direction. Mindy now prices an opportunity by its **PSC** (Product/Service Code — *what is actually being bought*) instead of its broad NAICS (*who the seller nominally is*). A Bureau of Prisons buy for a handful of Motorola radios carries the broad IT-services NAICS 541519, but its PSC says "radios" — so Mindy prices it against real radio contracts (~$2M), not a $77M IT mega-vehicle. The estimate leans on the true predecessor contract when there's a confident same-product match, falls back to a comparable-award band when there isn't, keeps a **real large value** when the prior contract is a genuine same-class recompete (an $82M dredging job stays $82M), and stays silent rather than guessing when nothing fits. Recompetes — which already have a real awarded value — show that number directly and never get a modeled estimate. The result: across **14,367 live opportunities, zero** now show an implausible figure (the maximum any M-Estimate reaches is a real $99.7M naval contract), down from **237** that had been showing fabricated values as high as **$42 billion**.

**Why:** An estimate that's off by 1,000× isn't a rounding error — it's the difference between a small business bidding and walking away. The core insight is one every GovCon veteran knows and most tools miss: **NAICS is who the seller is; PSC is what was bought** — and for pricing, what was bought is the truth. NAICS codes are broad ("Other Computer Related Services" covers a $25,000 radio order and a $3.8 billion Army IT program alike), so pricing off NAICS lets a giant unrelated contract masquerade as the market. Pricing off PSC — the actual product or service class — anchors the number to real, comparable work. Just as important is knowing when *not* to show a number: a modeled estimate and a real awarded contract value are different things, so a recompete shows its firm figure while an open opportunity shows an estimate, and when the data doesn't support a confident number Mindy says so instead of fabricating one. Contractors can price a pursuit at a glance and trust it — the estimate reflects the actual product market, not an artifact of the classification system.

**SEO:** federal contract value estimate, GovCon opportunity pricing, PSC vs NAICS, how much is a government contract worth, contract award value lookup, incumbent contract value, federal market pricing intelligence, place of performance contract estimate.

**Proof:** Every figure was verified against **live USASpending data**, not a green build. Worked examples, each confirmed live: a BOP APX-radio buy dropped from a fabricated **$77.1M** (a wrongly-matched L3Harris IT IDV) to a real **~$2M** with the correct radio incumbent; "Kitchen Equipment for the Missouri National Guard" dropped from **$11.6 billion** (matched to Sikorsky's National Guard *aircraft* contract) to **$9M** with a real kitchen contractor; a real **$82M** Newark Bay dredging recompete correctly *keeps* its $82M (its predecessor is genuinely the prior dredging of the same channel). Grounded-not-guessed throughout: the estimate is computed from real federal award amounts (capped so a governmentwide IDIQ ceiling can't pose as one opportunity), a recompete shows its actual award figure, and when no confident match exists the card shows an honest comparable-award band or nothing — never a fabricated number. The whole-population sweep confirmed **0 of 14,367** live opportunities show an implausible estimate. Guarded by unit tests plus a live oracle that re-checks the branded number against ground truth.

---

## The map pin popup is now an "Expanded Decision Card" — six fields, one action, no database dump (2026-08-04)

**What:** Click any pin on the Opportunity Map and the popup is now a compact **decision card**, not a mini spreadsheet. It answers one question — *is this worth opening the full listing?* — in six calm slots: a **lifecycle header** (Open now · Recompete · Forecast, with the horizon date on its own quiet line), the **buyer's identity** (the sub-agency plus its strongest signal, e.g. "Army · Repeat Buyer"), the **value**, the **title** (clamped to two lines so every card shares the same rhythm), **one plain-English sentence** built from the record's own fields ("Due Aug 17 · Posted Jul 23", "Incumbent expires Mar 2027", "Planned for Q2 FY2026"), and **one action button that matches the lifecycle** — Track Forecast, Review Opportunity, or Analyze Recompete. Everything heavier — the exact NAICS, the response-due grid, the solicitation number, the "View on SAM" link — now lives one level deeper, on the full listing, where it belongs. A **forecast** shows the agency's **own published estimate range** (e.g. "$2M–$7.5M") as a real government figure with no "≈"; only an **open** opportunity with no published number shows Mindy's modeled "≈ M-Estimate"; a **recompete** shows its real contract value. Same visual language as the result-list card and the full listing, so the flow reads as one system: **Map → Decision Card → Expanded Decision Card → Listing → Pursuit**.

**Why:** A popup crammed with every field forces the reader to *hunt* for the one thing that decides whether to keep going — so it decides nothing. The rule that shaped this release: **every field on the popup must justify why it helps someone decide whether to open the full listing; if it doesn't change that decision, it lives one level deeper.** That's why the four-box grid and the solicitation UUID came out, why the title is clamped so long titles can't break the rhythm, and why the action button names the actual next step instead of a generic "Should I bid?". It also fixes an honesty gap: a forecast is *planned* work the agency has itself put a dollar figure on, so showing that figure straight (no "≈") is more truthful than layering Mindy's model on top of a number the government already published. The estimate leads, but it no longer *overwhelms* — it's sized so the eye reads identity → number → title, in that order.

**SEO:** federal opportunity preview card, contract decision card, government forecast value, agency published estimate, opportunity map popup, GovCon lifecycle Open Recompete Forecast, should I bid federal contract.

**Proof:** Grounded in real data end-to-end. The forecast value is the agency's own `estimated_value_range` from the forecast record (populated on 97.6% of forecasts; verified against the live database), shown verbatim — never a modeled or invented number, and an honest "Estimate pending" when the agency published none. The one context sentence is assembled purely from the record's own dated fields, never written by an AI. The redesign is also **instrumented**: every popup open and every action-button click is logged (signed-in users, fire-and-forget) with the opportunity's lifecycle and identity, so the product can now measure which listings get opened, which action buttons get clicked, and which lifecycle converts — the data that replaces opinion about what to build next. Verified: the nine new interface elements are present in the generated map, the forecast value columns exist on the forecast table, and TypeScript compiles clean; live pin verification on production follows the deploy.

---

## The opportunity listing is now a decision workspace, not a database record (2026-08-04)

**What:** Open a listing and the page reads like a capture manager's actual thought process, top to bottom — **Snapshot** (what is this?) → **🎯 Should I Pursue This?** (is it worth chasing?) → **Opportunity Intelligence** (what's being requested?) → **Market Intelligence** (what does the market look like?) → **Buyer Intelligence** (who am I selling to?) → **Decision Makers** (who should I know?) → **✍️ Win This Contract** (how do I execute?) → **Related Opportunities** (what else?). The sticky **Next Actions** bar (Start Pursuit · Generate Proposal · View on SAM) stays pinned to the bottom no matter how far you scroll. A one-tap tab rail follows the same eight questions.

**Why:** The card answers "should I *open* this?" The listing answers a different, bigger question: "should I *pursue* this?" — so every section has to earn its place against that decision. The order encodes a real capture sequence: **Understand → Decide → Research → Know the Buyer → Execute.** Two deliberate calls make it sharper. "Win This Contract" now sits *above* "Related Opportunities," because once you've decided to pursue, your next thought is *help me win* — not *show me something else*; execution is the goal, related listings are a browsing convenience, so execution comes first. And "Next Actions" isn't a section you scroll to — it's always there at the bottom, so the primary action is one tap away from anywhere on the page. The guiding rule, stamped at the top of the spec: the listing is not a database record, it's a decision workspace — every field must help someone decide whether to pursue, or help them win.

**SEO:** federal opportunity listing, should I bid on this contract, GovCon capture workflow, bid decision workspace, pursue or pass federal contract, opportunity decision flow.

**Proof:** The reorder is content-and-labels only — the page's layout, column widths, spacing, and type scale are untouched (verified: the change added zero sizing or layout code). Every section still renders its real, grounded data — the M-Estimate from comparable federal awards, the incumbent and contract history from USASpending, the decision makers from the real office roster — in the new order. The "Should I Pursue This?" analysis stays on-demand (it runs the real Bid/No-Go engine when you ask for it), so the page stays instant to open. Verified across the full test suite (1,862 tests) with the tab order and section sequence asserted, then confirmed live on production.

---

## M-Win™ joins M-Estimate™ in the listing hero — your personal odds, grounded in your profile (2026-08-04)

**What:** Every open-opportunity listing now leads with **two** branded numbers side by side: the **M-Estimate™** (what the work is worth) and the new **M-Win™** — your personal win-probability for that specific opportunity, shown as a percentage with a one-line "why" ("Win-probability · strong NAICS + set-aside fit"). It's computed from *your* profile — your NAICS, certifications and set-aside eligibility, target agencies, and typical contract size — against that opportunity. If you haven't set up a profile yet, the card is honest about it: instead of a made-up number, it shows "Complete your profile to unlock M-Win" with a one-tap link to set it up.

**Why:** M-Estimate answers "how big is this?" M-Win answers the more personal question a contractor actually asks: "what are *my* odds here?" Together they turn the listing hero into a two-second read — is this worth my size, and is it worth my odds. Crucially, M-Win is only ever a real number when it's grounded in your real profile; we never show a fabricated or generic percentage dressed up as personalized, because a wrong win score sends you chasing the wrong bid. No profile means no number — just an honest nudge to set one up. And the hero stays instant: M-Win loads on its own, so the M-Estimate and the rest of the page never wait on it.

**SEO:** federal contract win probability, M-Win score, should I bid win odds, GovCon win probability by profile, personalized bid fit score, set-aside match score.

**Proof:** M-Win is the *same* win-probability model that powers Mindy's scoring elsewhere — the one guarded by our automated oracle (a perfect profile-to-opportunity match scores 98, a no-profile call falls back to a fixed baseline, and an unrelated opportunity always scores lower than a strong match). The listing hero calls that exact model, so the number you see here matches M-Win everywhere else — no second, divergent formula. Every factor traces to your real profile data (NAICS match, set-aside eligibility, agency experience, contract-size fit) and to the opportunity's real fields; nothing is invented. When we can't ground it — signed out, no profile, or a compute hiccup — the card says so plainly rather than showing a number. Verified end to end: a signed-out or profile-less user never sees a percentage (only the honest "complete profile" card), a real profile produces the real score, and the whole path is covered by unit tests plus the win-probability oracle.

---

## "Should I Pursue This?" is now a filled decision card, not a button (2026-08-04)

**What:** The listing's signature section — "Should I Pursue This?" — used to be a single button you had to click. Now it arrives already answered: a **Pursue / Watch / Skip** recommendation with Mindy's one-line call, a **Why** column (the real strengths — your set-aside match, the buyer's repeat-buy history, a value that fits your past performance), a **Risks** column (short response windows, an entrenched incumbent), and a **Win factors** line. The deeper **Bid / No-Bid** AI analysis stays a button you press when you want Mindy's full written reasoning.

**Why:** This is the heart of the listing — the whole point is to help you decide whether to chase an opportunity, so that decision shouldn't be hidden behind a click. Everything shown before you press anything is **grounded**: it comes from the same win-probability model that produces your M-Win score, computed against your real profile — not an AI guess. Because it's the model's own factors (not a language-model call), it's instant and free to show on every listing, so the section is always populated with a real recommendation. The one thing that does cost a moment — the full written Bid/No-Bid reasoning — stays on-demand behind its button, so you get the at-a-glance call for free and the deep analysis only when you ask. And when we can't ground it (you're signed out or haven't set up a profile), the section shows just the button rather than a made-up recommendation.

**SEO:** should I bid on this contract, pursue or pass federal opportunity, bid no-bid decision, GovCon pursuit recommendation, win factors federal contract, capture decision workspace.

**Proof:** Every part of the pre-filled card traces to real data. The Pursue/Watch/Skip call is derived from your win-probability score; the Why and Risks lines are the actual positive and negative factors the model scored (NAICS match, set-aside eligibility, agency experience, contract-size fit) against your real profile and the opportunity's real fields; the Win factors are the named strengths that earned points. Nothing is invented — if there's no profile to score against, there's no recommendation, just the button. Verified by unit tests that pin the recommendation thresholds and the factor-to-Why/Risks mapping, plus the drawer guard that keeps the card behind the grounded check and preserves the on-demand Bid/No-Bid analysis.

---

## Pursuits is a work queue, not a CRM — the next action is the whole row (2026-08-05)

**What:** The Pursuits page ("mission control" for the opportunities you're actively working) was rebuilt so each row leads with the one thing that matters: **the next action**, rendered as the large, bold focal line. The opportunity title drops to a small line above it, the value moves to a quiet chip on the far right, and a meta line underneath shows the due-relative and a "**Updated N ago**" badge. The header KPIs dropped "Pipeline value" and now read **Active · Need attention · Due this week · Waiting** — the four numbers that describe *work*, not valuation. Health chips gained a reason ("At Risk · action overdue", "Stalled · no next step"). The subtitle is now "Know what needs your attention today," and the primary button stays "**Continue**."

**Why:** People open their pursuit list asking "what needs me *today*?" — not "what's my pipeline worth?" So the page optimizes for work: the next action dominates every row as a plain human sentence, the KPIs count things you can act on, and pipeline valuation moves to Reports where it belongs. "Continue" (not "Open") is deliberate — you're resuming work, not browsing a record. Where the data can't support a widget, we leave it out rather than fake it: there's no "waiting on customer" signal stored, so that tile is honestly labeled "Waiting" and counts by the same rule as the Waiting section, so the two always reconcile.

**SEO:** federal contract pursuit tracker, GovCon capture work queue, next action pipeline, bid pursuit management, what to work on today federal contracting, opportunity pursuit dashboard.

**Proof:** Grounded end to end. Every row's next action is the pursuit's real `next_action`, shown verbatim — with one honest guard: a handful of older rows carry an internal action-key (e.g. `request_pursuit_brief`) instead of a sentence, so those are mapped to a readable label or, if unknown, shown as "No next step set" — never a raw code, never an invented step. The health reason traces only to real fields (the next-action date, the response deadline, the priority); the "Updated N ago" badge reads the row's real `updated_at` and omits itself if absent rather than showing a fake time; the "Waiting" tile reuses the exact rule of the Waiting section so the count can't drift. Verified: TypeScript compiles clean, the derivation helpers are locked by unit tests (13/13, including the health-reason object and the action-key humanizer), a headless render of real-shaped rows passes 15/15 checks with zero JavaScript errors, and the full production build (546 pages) is green.

---

## The Watchlist Morning Brief now tells you *why* each market matters today (2026-08-05)

**What:** The Watchlist opens with an actionable read of your day — "64 new opportunities across 3 markets. Start where the biggest changes happened." — and three KPIs sized for planning: New opportunities, Current matched value, and **Closing this week** (not just today). Each saved market gets a **Today's Story**: a few grounded reasons it deserves your attention — *Repeat buyer*, *Small-business friendly*, *Early-stage (sources sought)*, *Closing soon* — plus a **recent-change line** ("1 deadline moved", "1 amendment") when Mindy has actually detected a change on an opportunity you track. Markets are named in plain English ("Cybersecurity," not "NAICS 541512"), and the button is dynamic: "Explore 61 New Opportunities →". You can sort by Most new, Most urgent, or Biggest market.

**Why:** A watchlist should answer "where do I spend my time this morning?" — so every element is a reason to act, and every number is real. The Today's Story reasons come straight from each opportunity's own signals (its repeat-buyer history, set-aside friendliness, lifecycle stage), and the change line comes from Mindy's live amendment tracker — the same one watching your pursuits — so it appears **only when a real change happened**, never as a hollow "0." Where the honest answer is "nothing new," the page says so plainly instead of inventing activity. Trend language ("up from yesterday," sparklines) is deliberately absent until enough daily history has accumulated to make it true.

**SEO:** federal opportunity watchlist, saved search alerts, contract deadline this week, why pursue this market, GovCon morning brief, amendment tracking federal contracts, repeat buyer signal.

**Proof:** Every figure is grounded end to end. New-opportunity counts, matched value (a sum of grounded M-Estimate medians, nulls skipped — never $0-filled), and "closing this week" all run through the same shared matching engine the daily alerts use, so the Watchlist can't disagree with your inbox. The Today's Story reasons are tallied from each matched opportunity's real DNA signals; the change line reads Mindy's actual amendment log (deadline moves, re-posted notices, cancellations) scoped to the opportunities you track, rendering only when a real row exists. What can't be grounded yet — "since yesterday," unread state, deltas — is left out rather than faked. Verified: TypeScript compiles clean, the grounding rules are locked by 14 unit tests, and a headless render of real-shaped data (including the honest "nothing new" and "no story" cases) produces zero errors.

---

## The Market Intelligence report now reads like a briefing, not a dashboard (2026-08-05)

**What:** The Reports page was resequenced to read top-to-bottom like a market briefing: the **Executive Brief** (renamed from "AI market summary"), then **Opportunity DNA** — Mindy's signature signal — promoted to sit directly beneath it, then the buyers, the money, the forecasts, and the biggest open opportunities. The DNA section swapped its pie chart for **ranked horizontal bars**, so you can compare strand mix at a glance (Repeat buyer, Full & open, Early cycle, Set-aside…) instead of squinting at wedges.

**Why:** A report should have a reading order — a headline, then the thing that makes it different, then the supporting detail — not three interchangeable dashboard tiles. Opportunity DNA is the differentiator, so it comes first, right under the brief. And horizontal bars beat a pie every time for comparison: your eye reads length instantly, where pie wedges force you to estimate angles. "Executive Brief" also replaces "AI market summary" because the value is the grounded intelligence, not the fact that a model helped write the sentence — the name should read premium, not like a chatbot.

**SEO:** federal market intelligence report, opportunity DNA, market briefing GovCon, contract market analysis, executive brief federal contracting, top buyers by agency.

**Proof:** This was a presentation pass — the reordering, the pie-to-bars swap, and the rename touch only layout and labels; every number is the same grounded figure from the same market-intelligence engine (total market value, top buying sub-agencies by obligated dollars, the DNA strand mix across real open opportunities, place-of-performance value, forecasts). The DNA bars are share-of-total percentages so their lengths are directly comparable, and the section still omits itself entirely when the data isn't there — never an invented chart. Verified: TypeScript compiles clean, no pie-chart rendering code remains, and the full production build passes.

---

## Market Intelligence reports now end with recommended actions, not just data (2026-08-05)

**What:** The Reports page got a final polish pass. It now reads in a clean narrative order — Executive Brief → Opportunity DNA → Top Buyers → Forecasts → Largest Open Opportunities → Where the Money Is (geography last) — and **ends with a "Recommended actions" list**: a few concrete next steps like *Watch <top agency> · Monitor <N> upcoming forecasts · Focus on <top state> · Review <N> upcoming recompetes*. The Saved Markets rail on the left now shows each market's total value like a watchlist, and the "SB-friendly buyers" metric reads "None identified" when it's genuinely zero instead of a bare "0" that looked broken.

**Why:** A market report should end with what to *do*, not trail off in a data table — so the last thing you read is an action list, and every action points back to the map. Those recommendations are generated entirely from the report's own numbers — the biggest buyer, the real forecast count, the top state by value, the number of expiring contracts — so they're advice you can trust, not a language model's guess. The value-per-market rail turns Saved Markets from a settings list into a portfolio view. And "None identified" is simply the honest way to show a real zero without it reading as an error.
...
**What:** A terminology pass so each name in Mindy means exactly one thing. **Today's Intel** is the homepage briefing ("what changed today?"). **Today's Lens** is the preconfigured map view that opens already focused on the day's signals ("how should I explore?") — and the button that opens it now says "Open Today's Lens" everywhere, matching the label you see once you arrive, instead of the old "Open Today's Map." **Market Signals** are the grounded facts on each Watchlist card ("why did this market move?"). The email briefings' news section, which used to also be called "Market Signals," is now "Market Highlights" — because the headlines that matter to your market are a different thing from the buyer-behavior signals on a card.
...
## Mindy for Government — the APEX pages (/gov · /institute · /pilot)

**What:** Three new agency-facing pages on getmindy.ai — a government landing page (`/gov`), a one-page research institute (`/institute`), and a pilot-program page (`/pilot`) — under the banner "The Mindy Institute for Public Procurement." They exist to give public-agency buyers (at events like APEX) a 30-second understanding of what Mindy does for *agencies*: reach the qualified small businesses their traditional procurement channels are missing.

**Why:** Most vendors tell an agency "we help contractors find opportunities" or "we help you post solicitations." Mindy says something different and more defensible: **agencies don't have a posting problem — they have a discovery problem.** Posting is the easy part; making sure the right suppliers (especially first-time small businesses) actually *find* a solicitation is where competition is won or lost. This reframe is the whole positioning — a problem the industry has quietly accepted for years — and it's grounded in the government's own numbers, not a pitch.

**SEO:** supplier discovery for government, increase competition in federal procurement, reach qualified small businesses, small business participation, procurement outcomes, supplier intelligence, government contracting pilot, competition gap, new-entrant decline.

**Proof:** Every statistic on these pages is real and sourced in a footer that appears on all three: a **38%** decline in small businesses selling common goods/services to the government (2010–2019) and a **79%** decline in *new* entrants (2005–2019), both from the Bipartisan Policy Center's 2021 procurement-reform report drawing on CSIS + SBA data; DoD small-business vendors falling **43%** (42,723 → 24,296, 2011–2020) from GAO‑22‑104621; the FY2023 small-business share of **28.4% / $178.6B** from the SBA Procurement Scorecard; and OMB memo M‑23‑11 ("Creating a More Diverse and Resilient Federal Marketplace") as the supplier-base priority. The narrative deliberately does **not** claim agencies fail the 23% goal (they exceed it) — the honest, harder-to-dismiss point is that the dollars flow to a *shrinking* pool. No figure is modeled or projected; the pilot page's case-study block is explicitly labeled a template, not results. The pages are positioning only — no app, auth, or database coupling — with a distinct civic-navy + oxide-teal identity separate from the contractor product, and the real Newsreader + Inter typefaces are embedded so an agency officer's browser renders them exactly as designed. Verified: TypeScript compiles clean and a headless render of all three routes shows zero JavaScript errors with the embedded fonts and full sourced content intact.

---

## The Mindy Institute research library (/institute)

**What:** Four new grounded white papers published as designed, downloadable pages under `/institute` — turning the Institute into a real publications hub alongside the live Competition Gap paper. **No. 2 — The MLS Problem in Public Procurement** (bid notices are public by law, but the path to them runs through paid intermediaries — the Zillow/MLS analogy). **No. 3 — 90,887 Front Doors** (federal has one place to look; state and local has ninety thousand purchasing entities and no equivalent). **No. 4 — Who Isn't Bidding on Your City's Contracts** (the #1 reason firms don't bid is that they never heard about it). **No. 5 — The Bidder Pool Is Shrinking** (DOT-funded research names bidder outreach as the highest-return fix, and 70% of states don't do it).

**Why:** These extend Mindy's thesis from the federal/small-business world into **state and local government** — cities, counties, and special districts — which is exactly where the audience and the early adopters are. The MLS/Zillow framing is the same metaphor the whole product is built on: public information exists, but a paid layer sits between the public and its own records. The papers make the case to city and county leadership that reaching more qualified firms is a *pricing* lever, not a fairness talking point.

**SEO:** public procurement bid notices, why firms don't bid, thin bidder pool, bidder outreach cost savings, municipal solicitation distribution, DemandStar alternative free bid posting, 90,887 governments, open contracting, state and local procurement.

**Proof:** Every figure and quotation is the author's own, cited to a public source — the 2022 Census of Governments (90,887 entities), GAO's FY2024 contracting snapshot ($755B), the DOT/NBER Liscow-Nober-Slattery study (the 17.6% / 70% findings, with pinpoint page numbers and the Romano-Wolf adjusted p = 0.01), the Raleigh and Boston disparity studies (presented *with* their own sample-size caveats), and the access-layer companies' own published pricing pages, FAQs, terms of use, and supplier marketing. The papers are unusually careful — they refuse to invent a state/local procurement dollar figure because none authoritatively exists, they note where the Zillow analogy breaks, and they flag the litigation example's limits. That rigor is the point: it reads as research, not vendor marketing. Each paper renders from its markdown verbatim into the same designed, printable page as the live Competition Gap paper, with a Download-PDF button. Verified: TypeScript compiles clean, and a headless render of all four routes returns HTTP 200 with the embedded fonts, tables, blockquotes, and full sourced content — zero JavaScript errors.

---

## The Mindy Institute — one Institute, honestly labeled (2026-08-07)

**What:** Consolidated two conflicting research surfaces into one. `/research` is now the single canonical **Mindy Institute** — publications that cite permanent Observatory standards (OBS-###). The older `/institute` gallery of six ungrounded marketing white papers is reframed as a public **Research Backlog**: each is relabeled a **Research Concept**, states the claim it believes the evidence will support, and names the Observatory standard it awaits (or "standard not yet defined" where none exists). Every concept carries a visible status banner and a *Publication Status* footer linking to the maturity ladder at `/research/how-we-publish`. `/research` gained a **Research Agenda** section that lists the concepts by the standard each awaits; `/research/about` now encodes a numbered **3-rule editorial charter**.

**Why:** Authority compounds when it traces to a defensible measure and fractures when two "Institutes" disagree. The rule the whole surface now enforces (Eric): *the Institute publishes a conclusion only when the supporting Observatory standards are mature.* The six papers weren't deleted or hidden — the principle is "never hide truthful content, always label it accurately." A concept kept online and honestly marked is an intellectual trail: it shows the question predated the evidence, and it stays archived as a precursor even after the real publication ships.
...
## Opportunity Map — search several industries at once, without guessing the syntax (2026-08-14)

**What:** The NAICS filter on the Opportunity Map now works like the tag fields people already know. Type or paste codes however you like — `541512 541611`, comma-separated, or one per line — and each becomes a labeled chip showing the code *and* its official industry name. Type a word like "construction" and pick from the matching codes. Chips are removable; the map filters on exactly what you see.

**Why:** The old field was a single text box that accepted anything and split only on commas. Nothing told you multiple codes were allowed, let alone that they needed commas — so typing two codes with a space between them, or a word you never selected, quietly returned the wrong opportunities. No error, no empty state, just a plausible-looking map built on a filter that meant nothing. A filter that fails loudly is a nuisance; one that fails silently costs a contractor the bid they never saw.
...
## Industry days, where you'll actually see them (2026-08-14)

**What:** Federal industry days, pre-bid conferences and forecast sessions now surface in three places they were missing. On an **opportunity**, an "Upcoming events" section shows events you can still attend — labeled with why it matched ("Matched to this solicitation" vs "Matched to buying office"). On the **Network** and in **Market Research**, past events become buyer-behavior signals: "Runs Industry Days — 7 in the past year."

**Why:** An industry day is one of the highest-leverage hours a contractor can spend — it's where you meet the people who write the requirement, before the RFP exists. Mindy had thousands of these on file and showed them nowhere. Splitting them by usefulness matters too: an event next month is something you *do*; an event last March tells you something about *how that office buys*. Only the first belongs on an opportunity you're evaluating today.
...
## Every link into the map now carries what you were looking at (2026-08-15)

**What:** Links that send you to the map no longer drop the thing that made you click. A market report's "Back to map" returns you to that same market. The browse hub's "Top buying agencies" and "Top markets" rows open the map already scoped to that agency or industry. A saved-search alert email — "12 new matches for *Construction — Open*" — now opens the map narrowed to that saved search rather than to everything, and each opportunity in it opens inside the map rather than bouncing you out to SAM.gov. And a featured opportunity on Today's Intel carries its own characteristics across: open one marked *Repeat Buyer · SB-Friendly · Set-Aside* and the map behind it shows the rest of the market that shares those traits.

**Why:** A link that discards its context makes you rebuild, by hand, the search you were already looking at — and most people don't; they leave. This is the difference between a briefing that points at the map and a briefing that configures it. It matters most on the highest-intent click there is: someone opening an alert email about a search they set up themselves. That person should land inside their search, not at the front door of a national database.

**SEO:** filtered federal contract search, saved search alerts government contracts, agency-specific contract opportunities, small business set-aside opportunity map, federal procurement by industry.

**Proof:** Every claim here was measured in a real browser against the live database, not asserted in a test. Against a 145,467-opportunity baseline: an agency link narrows to 62,099, an agency-and-state link to 804, and a featured card's characteristics to 135,222 — and the map's own filter controls visibly update to show what was applied, so the filtering can be checked rather than trusted. Only characteristics the map can genuinely filter on are ever put in a link: three real ones (including the second most common) have no corresponding filter, and a card carrying only those links without the parameter rather than shipping a link that looks filtered and isn't. Two defects were found and fixed by this measurement, both of which had been live: the "open the map" button in saved-search alert emails, and the characteristics filter itself — which had been silently applying nothing on every surface that used it, including the daily alert email, while the address bar showed a filter that was never applied.

## Today's Intel becomes a control panel for the map (2026-08-16)

**What:** Every number on the front page is now a door. Click "1,371 posted in the latest day" and the map opens showing exactly those — with the Posted filter visibly set to *Last 24 hours*, so you can see what was applied and change it. Click "116,552 contracts up for recompete" and the map switches to the recompete view. Click an agency, an industry, or a featured opportunity and the same thing happens. The one number that has no map view — upcoming industry events — is now shown as a plain figure rather than a link, because sending you to a map that cannot display events is worse than not offering the click.

**Why:** A homepage that only describes the market makes you rebuild, by hand, the view you were just reading about. Most people don't; they leave. The page should hand you a configured map, not a starting point — a briefing that creates a question and a map that answers it, in one click. That also means being honest about the exception: a link that quietly does nothing costs more trust than a number that never claimed to be a link.
...
**Follow-up (same day) — empty is knowledge, failure is unknown.** Mindy now tells apart "we measured this market and found no federal awards" (a real, billable answer) from "we couldn't measure it" (not billed). Hosted verification showed an empty market being treated as a system failure and not charged; the same distinction now holds on the NAICS path, for malformed warehouse responses, and for a solicitation lookup that reports its own failure. Proof: 11 regressions that fail when the fixes are reverted, and the real credit ledger reconciling empty-market and no-match answers at the normal price while failures stay at 0.

## POTETO — Compliance Matrix Truth (2026-09-22)

**What.** Mindy's compliance matrix now proves every row. Each requirement it returns carries the solicitation's own words, found in a named document at an exact location. It cites a section only when that section is printed in the document. Anything Mindy could not verify is listed separately with the reason, never mixed into the trusted matrix. Mindy also says how much of the package it actually read, document by document.

**Why.** "Tell me what the solicitation actually requires, show me where it came from, and never invent a requirement, quote, section or citation." A compliance matrix is a high-trust artifact: one invented "Section L" or a misquoted submittal can send a proposal team after something the government never asked for.
