...
**SEO / proof.** App merge: `preserveRicherEnrichment`. DB: `trg_recompete_preserve_enrichment` BEFORE UPDATE. Phase 2 BQ refill (2026-09-17, SHA `cf76fc6f` prevention live): UPDATE-only from `award_detail_lookup`, never overwrite non-null. Attempted 98,511 / updated 98,511 / failed 0. PSC present 4,385 → 102,880. Description present 4,369 → 102,875. Both-null 136,514 → 38,021. Table rows not created by this write (140,899 → 140,901 from hourly sync inserts). Live `get_expiring_contracts` 541910/36/50: PSC+description 0/50 → 44/50. Hourly sync overlap on 541512: 26 → 949 enriched rows; incoming nulls kept DF10/J059/5810/N059/DB02. Remaining USASpending-detail queue = 38,021 unstamped both-null. Script: `scripts/refill-recompete-enrichment.ts`.
