...
| 2026-07-27 | Rename -> Recompetes | "Awarded" sounded finished, "Contract Vehicles" was jargon; these are ACTIVE contracts you learn about from the award record. Renamed to "Recompetes" across nav + dropdown + header (names the action, not the lifecycle state) | `title:'Recompetes'` -> `src/app/opportunity-map/route.ts` | 3 tests | 🟡 IN REVIEW (branch fix/rename-recompetes) |
| 2026-07-27 | Recompete two-play CTA | A recompete row is TWO plays: running TASK ORDER -> SUBCONTRACT to incumbent; expiring PRIME -> get ahead of the recompete. CTA + Claude prompt now match the play (Plan outreach vs Plan recompete vs Start drafting) instead of always "Start drafting" | `function draftCTA(o)` -> `src/app/opportunity-map/template.html` | 4 tests; 117 map tests | 🟡 IN REVIEW (branch fix/rename-recompetes) |
