...
| 2026-07-27 | Home page | Removed the temporary 'Beta user? Already getting Mindy alerts — set up your account' card (email-only-cohort onboarding). Recovery now handled everywhere (Sign in + Forgot password), so the standalone card is redundant | `MindySignupForm` (beta card gone above it) → `src/app/mindy-landing/page.tsx` | tsc clean | 🟡 IN REVIEW (branch fix/remove-beta-setup-card) |
