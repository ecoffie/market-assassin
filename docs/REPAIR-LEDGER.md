...
| 2026-07-27 | Map · Filters panel | Density/typography pass (unpaused): fixed the still-live placeholder bug (JS \u201C escape in HTML attr rendered literal); removed the DUPLICATE .mf-sec rule ('different fonts'); tighter Zillow-density groups w/ hairline top-dividers | `border-top:1px solid var(--hair)}` → `src/app/opportunity-map/route.ts` | 99 map tests; preview visual-verify pending | 🟡 IN REVIEW (branch feat/filters-panel-density) |
