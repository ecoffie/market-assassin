...
| 2026-07-27 | Recompete pill on every card | "Recompete now" stamped on EVERY Vehicles card (redundant with Expires cell; "not how Zillow does it" — Zillow shows the sold date, not a verb on every card). Pill now shows ONLY in the actionable window (warm: "Recompete now"/"Recompete window"); the "Active — subcontract" majority drops it, Expires carries it | `f.c==='hot' || (o.src==='RECOMPETE' && f.c==='warm')` -> `src/app/opportunity-map/template.html` | 3 tests; 113 map tests | 🟡 IN REVIEW (branch fix/vehicles-naming-recompete-pill) |
| 2026-07-27 | Awarded -> Contract Vehicles | "Awarded Contracts" read too narrow for a corpus of IDIQs/task orders/expiring recompetes; renamed to "Contract Vehicles" (the broad overview) across nav + dropdown + header | `title:'Contract Vehicles'` -> `src/app/opportunity-map/route.ts` | 3 tests | 🟡 IN REVIEW (branch fix/vehicles-naming-recompete-pill) |
