...
| 2026-07-27 | Map · Company list cards | Company/Buyer sidebar cards brought to Awarded-card polish: color strip + chip + title + meta + a real .stats facts GRID (Won/Awards/Agencies) + footer with 'View details' (was chip+title only) | `class="stats"` → `src/app/opportunity-map/route.ts` | 99 map tests | 🟡 IN REVIEW (branch feat/polish-company-list-cards) |
