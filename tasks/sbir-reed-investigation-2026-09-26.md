...
> **⛔ FINAL STATUS (2026-09-26): SBIR/STTR search is RETIRED on every customer-facing surface — PR #1710.**
> MCP `search_sbir`, the in-app SBIR panel and its sidebar item, `/api/sbir` (410), the Opportunity Map's
> SBIR source and `/api/market-scan`'s SBIR section. Record: `tasks/search-sbir-retirement-2026-09-26.md`.
>
> **PR #1708 was CLOSED UNMERGED.** Its library repair (§6) and tests had **no retained consumer**:
> `src/lib/sbir/search.ts` is imported only by the unregistered MCP wrapper, which nothing imports.
> The code stays on branch `fix/sbir-open-topics-vs-award-history` for reference; it was not merged
> just because it had been built.
>
> This packet moved into #1710 as the investigation evidence. Sections that describe #1708 code
> (§5, §6, §8) describe that closed branch, **not** `main`. The billing items (§9) remain proposals:
> - P1 — Eric recommends returning all 105 credits; it is NOT authorized by either PR.
> - P2 — leave the playbook charges unchanged.
> - P3 — moot for a retired tool.

*(Historical — the scope of closed PR #1708:)* **Release scope: a RESULT-CLASSIFICATION and FAILURE-REPORTING repair.** It does **not**
...
Separate batch from #1692. *(Historical: #1708 was in review at the time; it is now closed unmerged.)* No production-data
