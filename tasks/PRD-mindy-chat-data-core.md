...
| **Tier 1 — SHARED PLATFORM** | SAM opps, forecasts, contractor/award, recompetes | `sam_opportunities`, `agency_forecasts`, BQ `recipients`/`usaspending.awards`, `recompete_opportunities` | Public federal data — no per-user scoping. Normal query, cache-gated for BigQuery. |
...
- **Phase 2 — live market read (2 days):** `search_sam_opportunities` over `search_tsv` (+ optional semantic `hidden-match`).
