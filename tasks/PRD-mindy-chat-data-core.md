...
- **2026-07-15 — ✅ RE-VERIFIED WORKING AS DESIGNED (all 3 tiers).** Full regression check that every v2 tool still functions: **43/43 chat unit tests green** (`src/lib/chat/` — tier0/1/2 + starter-prompts, incl. the isolation + no-fabrication contracts) AND **8/8 live E2E** against a running dev server (`scripts/verify-chat-v2-e2e.mjs`, real `/api/app/chat` route, two real Pro users, signed session tokens). Confirmed each of the 6 tools fires and grounds on real Data Core data: Tier-0 `get_my_pipeline` (returned the caller's real pursuits) + `search_my_vault`; Tier-0 isolation (empty user B never saw A's rows) + no-fabrication on empty; Tier-1 `get_market_vocabulary` (real 541512 buyer-words) + `search_sam_opportunities` (real open SAM opps, no punt); Tier-2 `get_contractor_profile` (real Leidos intel — $16.6B / 11,000+ contracts) + `find_capable_contractors` (real 541512 teaming firms). Tool round confirmed running Sonnet 5 (gpt-4o → Groq fallback), each call routed to its owning tier by name, best-effort with the v1 RAG path preserved on any tool failure. No code change — verification only.
...
- **2026-07-11 — ✅ PHASE 3 SHIPPED (final tier).** Tier-2 (BigQuery contractor/award intel) tools: `get_contractor_profile` (a named company's federal award total, top agencies, recent contracts) + `find_capable_contractors` (firms winning a NAICS/PSC — teaming/competition). `src/lib/chat/tier2-tools.ts`. **Cost-disciplined** (the June-2026 $2,075 BQ spike): cache-only Pass 1 (free, no rate-limit), cold live-BQ Pass 2 only under a per-user hourly limit (`checkRateLimit`, 12/hr) AND a per-turn cap (2 cold lookups/message); over budget → friendly "slow down" note, never another scan. Warm hits cost nothing and don't consume budget. Bug found + fixed during E2E: a bare-name slug ("leidos") missed the stored "LEIDOS, INC." → now sweeps suffix slug variants + falls back to the single-recipient table via `getRollupOrSingleBySlug` (Leidos now resolves to real $16.6B intel, was "couldn't find"). 11 cost-discipline unit tests + **8/8 live-route E2E** (real Leidos profile + real 541512 teaming partners). 95/95 total tests, full build green. **v2 tool roadmap complete** (Tier 0 + 1 + 2 all live).
