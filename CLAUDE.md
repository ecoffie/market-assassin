...
| `npm run verify:search` | script | ORACLE test for every search surface — hits the LIVE DB + asserts CORRECTNESS vs ground truth (not just non-empty): a real term returns notices that genuinely contain it; a garbage term returns 0 (no fabrication); a code-like term word-boundary-matches (M7 ≠ M776); **Andre's multi-word query "cyber cloud compliance network server"** returns many rows (regression guard — it once returned 0) and the top-10 match MORE terms than the bottom-10 (ranking works); location "FL" is genuinely pop-OR-office FL. Exits non-zero on any mismatch. `-- --json` machine-readable. |
...
| `npm run usage:dist` | script | The Basic/Medium/Pro EVIDENCE PASS, made repeatable so it is re-RUN not re-derived. Read-only. Reports 30/60/90-day windows and, per customer-understandable action (listings opened · map actions · pursuits · proposal actions · MCP calls · Market Match · Market Reports · Pursuit Dossiers), unique users + P25/P50/P75/P90/P95/max. ⚠️ **CREDITS NEVER DEFINE THE CLUSTERS** — light/regular/heavy is segmented on ACTIONS alone; credits are a separate internal-economics column and "% currently paid" prints as an OUTCOME CHECK, never an input (segmenting on credits would make the tiers a restatement of the current price list instead of a measurement of behaviour). Measured 2026-09-08: paid share rose **5.0% → 15.3% → 57.9%** across clusters that never saw a credit or a subscription flag — that validation is the whole point. Caveats PRINT with every run: `user_engagement` begins 2026-04-28 (a 90-day window is nearly the full history, so there is no prior-period baseline); infrastructure cost per action is NOT available from these tables; **latency is not cost**; and "—" means UNMEASURED, never 0 (a surface with no rows is ambiguous between "nobody did it" and "the emitter was renamed"). `-- --days N` for one window; `-- --json` machine-readable. Needs `.env.local` + the `readonly_select` RPC; a failure exits non-zero and is NEVER reported as zero usage. |
| `npm run tier:replay` | script | **SIMULATION ONLY — its tier outputs are scenarios, NOT recommendations and NOT production configuration** (live pricing is `src/lib/mcp/packages.ts`; never copy a number from this script into it). Replays real 30-day behaviour against candidate plan shapes and reports fit, ceiling breaches, internal credits, revenue exposure and the extreme tail. Two scenarios are kept IN THE FILE so future runs stay comparable: **combined behavioural score** (listings+maps+pursuits+proposal+MCP all meter) and **MCP-only** (cheap interactions unmetered). Read-only by construction — no insert/update/upsert/delete/rpc path exists, and it writes no pricing, credit, entitlement or checkout state. Sources are paged to EXHAUSTION with `count:exact` reconciliation printed every run (a truncated read would mis-size every tier); every query binds `error` and THROWS, so a failed read is never reported as an empty result. Staff/admin/advocate accounts are excluded EXPLICITLY via the shared `getStaffRole`/`isAdvocateAccount` helpers — they are comped and would inflate both the tail and cost-per-dollar (measured 2026-09-08: excluding them removed the single largest tail account, which was staff). Paid status is an OUTCOME column, never an input to tier assignment. `-- --days=N`, `-- --json`. |
