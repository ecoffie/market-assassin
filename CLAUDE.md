...
- ⛔ Do not fix Maps/saved-search search locally — migrate the consumer. The **cross-surface gate**
  ...
  `toPlan` adapter and CI enforces MCP-equal meaning.
- MCP changes are proven by `scripts/mcp-find-replay.ts` → `scripts/mcp-find-replay-diff.ts` (exits 1 on any
...
- **`/admin/data-inventory` is an ADMIN TRUTH SURFACE (rewritten 2026-09-26, audit `tasks/data-inventory-audit-2026-09-26.md`).** Semantics live in `src/lib/data-core/inventory-model.ts`: every dataset has ONE `kind` (`source_corpus` · `derived_intelligence` · `derived_index` · `static_manual` · `passthrough`) and ONLY `source_corpus` enters the source totals, split BY GRAIN: owned source RECORDS (primary) + award TRANSACTIONS (`grain: 'transaction'`, the BQ warehouse) = persisted source rows. Never call it "unique" — datasets are summed per store, not deduplicated across each other (a test fails on the word). Stored ≠ served (SAM active, recompete `quality_flag IS NULL`, contacts `contact_kind = government_buyer`). A "source" is an UPSTREAM PUBLISHER, never a label. Registry counts that disagree are shown as DEBT, never adopted. ⚠️ `embedding_source='none'` rows carry an EMPTY-ARRAY sentinel, not a vector — `sow_embedding IS NOT NULL` over-counts the index by ~55K. ⚠️ The BQ `awards` table is TRANSACTION grain (65M rows), not distinct awards. `inventory-truth.unit.test.ts` derives candidate collectors from `src/app/api/cron/` — a new `institute-*`/`sync-*` collector fails CI until it has a row.
