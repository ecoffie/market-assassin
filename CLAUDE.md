...
| `npm run verify:maps-account` | script | Required Maps chrome regression. HMAC sessions decode `payload.sig` (`b64json(parts[0])`, not JWT `[1]`); broken photos fall back to initials, never `?`; homepage `/` + `/today` and `/opportunity-map` inject the shared account menu; `/api/app/me` reads `requireMIAuthSession`; OAuth pictures come from `identities[].identity_data`; Markets → Players is `?mode=companies` (no `buyers` option, remap before `#fltDataset`). Offline source is the merge gate (pre-push 4b + CI). `-- --live` curls getmindy.ai HTML (and HMAC `/api/app/me` when local secrets match prod). Would have caught the 2026-09-14 `?` avatar and blank Players pill, and a later main deploy overwriting them. |
